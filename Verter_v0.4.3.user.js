// ==UserScript==
// @name         Verter_v0.4.3
// @description  Auto-trade + Signals (EMA3/9, EMA9/15, SQZ) + CLOSE via legacy Trades observer (guard+dedupe) + BalanceStable(2s) + DockBox (+50%) + Stats (Wager on DOM close) + One-At-A-Time Gate + PCS-8 v3.1 (Colon-Zero Lock) + Cycles history (app102-port, colored in DockBox)
// @version      0.4.3
...
let vx_currentProfit = 0;          // PnL текущего цикла (как app102 currentProfit)
let vx_maxStepInCycle = 0;         // max шаг в текущем цикле
let vx_cyclesStats = [];           // массив итогов циклов (как app102 cyclesStats)
let vx_totalProfit = 0;            // суммарный итог по всем завершённым циклам
let vx_prevBal = null;             // для delta-баланса, если pnl в onClose = null
let vx_cyclesHost = null;          // контейнер истории внутри DockBox

// лимиты цикла «как в app102» (скопировать значения из SOURCE_APP102)
const vx_limits = {
  win: /* <LIMIT_WIN из app102, число> */ 10,
  loss: /* <LIMIT_LOSS из app102, число> */ 20
};

function vx_round2(x){ return Math.round((+x)*100)/100; }
function vx_safeDelta(finalBal){
  if (finalBal==null || Number.isNaN(+finalBal)) return 0;
  const d = (vx_prevBal==null) ? 0 : (+finalBal - +vx_prevBal);
  vx_prevBal = +finalBal;
  return vx_round2(d);
}
function vx_resetCycleApp102(){
  // переносим PnL цикла в агрегат и пересчитываем total (как app102 resetCycle)
  vx_cyclesStats.push(vx_round2(vx_currentProfit));
  vx_totalProfit = vx_round2(vx_cyclesStats.reduce((a,b)=>a+b,0));
  vx_currentProfit = 0;
  vx_maxStepInCycle = 0;
}
function vx_makeCycleSpan(status, profit, maxStep){
  const span = document.createElement('span');
  span.className = `vx-cycle ${status==='win'?'win':'lose'}`;
  span.textContent = `${String(status).toUpperCase()} P=${vx_round2(profit)} S=${maxStep}`;
  return span;
}
...
(function(){
  ...
  css.textContent = `
    ...
    .vx-cycle.win{ color:#2ecc71; }   /* зелёный текст для выигрышного цикла */
    .vx-cycle.lose{ color:#e74c3c; }  /* красный текст для проигрышного цикла */
  `;
...
(function(){
  // перехват onClose: (type, pnl, finalBal, amount)
  try{
    const _onClose = SessionStats.onClose;
    SessionStats.onClose = function(type, pnl, finalBal, amount){
      const r = _onClose.apply(this, arguments);
      try{
        // шаг
        const step = (typeof window.currentBetStep!=='undefined') ? (window.currentBetStep|0) : 0;
        if (step > vx_maxStepInCycle) vx_maxStepInCycle = step;

        // pnl сделки: при null считаем delta баланса
        const dealPnl = (pnl!=null) ? vx_round2(pnl) : vx_safeDelta(finalBal);
        if (finalBal!=null && !Number.isNaN(+finalBal)) vx_prevBal = +finalBal;

        // накопление PnL цикла
        vx_currentProfit = vx_round2(vx_currentProfit + (dealPnl||0));

        // триггеры завершения цикла (порт из app102)
        const endedWin  = (vx_currentProfit >= vx_limits.win);
        const endedLoss = ((vx_currentProfit - (+amount||0)) <= -vx_limits.loss);

        if (endedWin || endedLoss){
          const status = endedWin ? 'win' : 'lose';
          // запись в историю (цвет шрифта задаёт класс)
          if (vx_cyclesHost){
            vx_cyclesHost.appendChild(vx_makeCycleSpan(status, vx_currentProfit, vx_maxStepInCycle));
          }
          // перенос в агрегат и сбросы
          vx_resetCycleApp102();
        }

        // перерисовка шапки DockBox
        if (typeof DockBox!=='undefined' && DockBox.renderHeader) DockBox.renderHeader();
      }catch(e){}
      return r;
    };
  }catch(e){}
...
// [BEGIN BLOCK:CYCLES:DOCKBOX]
(function(){
  if (typeof window!=='undefined' && typeof DockBox!=='undefined' && !window.DockBox) {
    try{ window.DockBox = DockBox; }catch(e){}
  }
  if (!window.DockBox || !DockBox.renderHeader) return;
  const _renderHeader = DockBox.renderHeader;
  DockBox.renderHeader = function(){
    _renderHeader.apply(this, arguments);
    try{
      const host = document.querySelector('.verter-dock-header') ||
                   document.querySelector('[data-verter-header]');
      if (!host) return;
      let sum = host.querySelector('#vx-cycles-sum');
      let hist = host.querySelector('#vx-cycles-hist');
      if(!sum){ sum=document.createElement('span'); sum.id='vx-cycles-sum'; sum.style.marginLeft='8px'; host.appendChild(sum); }
      if(!hist){ hist=document.createElement('span'); hist.id='vx-cycles-hist'; hist.style.marginLeft='8px'; host.appendChild(hist); }
      vx_cyclesHost = hist;

      // сводка: MaxStep | Cycles | TotalProfit
      sum.textContent = `| MaxStep: ${vx_maxStepInCycle} | Cycles: ${vx_cyclesStats.length} | TotalProfit: ${vx_totalProfit}`;
      // история — уже добавляется по событиям, тут ничего не перерисовываем
    }catch(e){}
  };
})();
// /* [END BLOCK:CYCLES:DOCKBOX] */
