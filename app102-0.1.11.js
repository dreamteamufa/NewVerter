...
// [BEGIN BLOCK:IP_GRID_LAYOUT_V1]
(function IP_GRID_LAYOUT_V1(){
  'use strict';
  const p=document.getElementById('InfoPanel');
  const g=document.getElementById('graphs');
  if(!p||!g){ console.error('[IP][GRID] missing panel/graphs'); return; }
  const css = `
    #InfoPanel{display:grid;grid-template-columns:1fr 1.1fr;gap:12px;align-items:start}
    #InfoPanel>#graphs{grid-column:2;width:100%;height:360px;margin:0}
    #InfoPanel>:not(#graphs){grid-column:1}
    @media (max-width:1400px){#InfoPanel>#graphs{height:320px}}
  `;
  const st=document.createElement('style'); st.textContent=css; document.head.appendChild(st);
  console.log('[IP][GRID] layout applied');
})();
// [END BLOCK:IP_GRID_LAYOUT_V1]

// [BEGIN BLOCK:TF_GRID_PIN_V1]
(function TF_GRID_PIN_V1(){
  'use strict';
  const p=document.getElementById('InfoPanel');
  const g=document.getElementById('graphs');
  const tf=document.getElementById('tfSelect')||document.getElementById('tfSwitch');
  if(!p||!g||!tf) return;
  const css = `
    #InfoPanel>#${tf.id}{grid-column:2;justify-self:start;margin:0 0 8px 0}
  `;
  const st=document.createElement('style'); st.textContent=css; document.head.appendChild(st);
  console.log('[TF][GRID] pin applied');
})();
// [END BLOCK:TF_GRID_PIN_V1]

// [BEGIN BLOCK:PCS8_SELFAUDIT_SOFT_V1]
(function PCS8_SELFAUDIT_SOFT_V1(){
  const p=document.getElementById('InfoPanel');
  const g=document.getElementById('graphs');
  if(!p||!g||g.parentElement!==p) throw new Error('[PCS-8][STOP] UI invariant broken');
  setTimeout(()=>console.log('[PCS8][SELF_AUDIT t=0] PASS'),0);
  setTimeout(()=>console.log('[PCS8][SELF_AUDIT t=250] PASS'),250);
})();
// [END BLOCK:PCS8_SELFAUDIT_SOFT_V1]
...
SRC_SHA256=…
TARGET_SHA256=…
Anchors=OK HashFragments=OK SelfAudit(0/250)=OK
ReadonlyCRC=OK Syntax=OK ASTx2=OK ESLint=OK Prettier=OK
Result=PASS
*/
