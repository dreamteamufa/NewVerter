...
// [SSOT:APP:BEGIN]
const BUILD_NAME = 'Verter_v0.3.7';
const VER = '0.3.7';
...
(function () {
  ...
  var SignalEngine=(function(){
    var lastSqzOn=null;
    const CFG_EMA915={useMTF:true,m5Min:50};
    function getM5Closes(st){
      const arr=st.m5?.candles||[];
      const out=arr.map(function(c){return c.C;});
      if(st.m5?.cur) out.push(st.m5.cur.lastP);
      return out;
    }
    ...
    function evalAll(){
      ...
      if(closes.length>=15){
        ...
        if(sideRaw){
          var allow=true;
          if(CFG_EMA915.useMTF){
            if(m5Closes.length>=CFG_EMA915.m5Min){
              var ema20=ema(m5Closes,20);
              var ema50=ema(m5Closes,50);
              if(ema20.length && ema50.length){
                var fast20=ema20[ema20.length-1];
                var slow50=ema50[ema50.length-1];
                var m5Trend=null;
                if(fast20>slow50) m5Trend='BUY';
                else if(fast20<slow50) m5Trend='SELL';
                if(m5Trend!==sideRaw){
                  console.log('[SIG:EMA9/15][SKIP] reason=mtf_mismatch side=%s', sideRaw);
                  allow=false;
                }
              }else{
                allow=false;
