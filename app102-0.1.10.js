...
// @version     0.1.10
// @description  Fixed InfoPanel + integrated graphs + TF Select + minute-sync (PCS-8 v3.3 Zero-Fault)
...
function addUI() {
    ...
    const graphs = document.getElementById('graphs') || document.createElement('div');
    if (!graphs.id) { graphs.id = 'graphs'; }
    if (!graphs.parentElement) { newDiv.appendChild(graphs); }

// [BEGIN BLOCK:IP_LAYOUT_LMETRICS_RCHART_V1]
(function IP_LAYOUT_LMETRICS_RCHART_V1(){
  'use strict';
  const panel = document.getElementById('InfoPanel');
  if(!panel) { console.error('[IP] no InfoPanel'); return; }

  // контейнер строки
  let row = document.getElementById('ip-row');
  if(!row){ row = document.createElement('div'); row.id = 'ip-row'; panel.appendChild(row); }
  Object.assign(row.style,{ display:'flex', gap:'12px', alignItems:'flex-start' });

  // блоки
  let left  = document.getElementById('ip-left');  if(!left){  left  = document.createElement('div'); left.id  = 'ip-left';  row.appendChild(left); }
  let right = document.getElementById('ip-right'); if(!right){ right = document.createElement('div'); right.id = 'ip-right'; row.appendChild(right); }
  Object.assign(left.style,  { flex:'1 1 0',  minWidth:'420px' });
  Object.assign(right.style, { flex:'1.1 1 0', minWidth:'480px' });

  // собрать метрики слева: переносим по id, если существуют
  const ids = [
    'header-version','start-balance','start-price','mode','trading-symbol','reverse-flag','current-time',
    'cycle-profits','won-percent','wagers','profit-percent','sqz-indicator','price-trend','global-trend',
    'trade-dir','max-step','total-profit','cycles-history','history-box','tf-label'
  ];
  const alias = {
    ...
    'current-time':'time',
    'cycle-profits':'profit',
    'won-percent':'won-percent',
    'wagers':'wager',
    'profit-percent':'profit-percent',
    'sqz-indicator':'sqz-indicator',
    'price-trend':'price-trend',
    'global-trend':'global-trend',
    'trade-dir':'trade-dir',
    'max-step':'max-step',
    'total-profit':'total-profit',
    'cycles-history':'cycles-history',
    'history-box':'history-box',
    'tf-label':'tf-label'
  };
  for(const id of ids){
    const lookupId = alias[id] || id;
    const el = document.getElementById(lookupId);
    if(!el) continue;
    let target = el.id === id ? el : el.parentElement || el;
    if(!target.id) target.id = id;
    if(target.parentElement !== left) left.appendChild(target);
  }

  const leftovers = [];
  for(const node of Array.from(panel.childNodes)){
    if(node === row || node.id === 'ip-row') continue;
    if(node.id === 'graphs') continue;
    if(node.parentElement === left) continue;
    leftovers.push(node);
  }
  ...
  const graphs = document.getElementById('graphs') || (()=>{
    const g = document.createElement('div'); g.id='graphs'; return g;
  })();
  if(graphs.parentElement !== right) right.appendChild(graphs);

  // стили графика
  Object.assign(graphs.style,{ width:'100%', height:'360px', margin:'0', overflow:'hidden' });

  console.log('[PCS8][ANCHORS] OK [PCS8][HASH] verified');
})();
// [END BLOCK:IP_LAYOUT_LMETRICS_RCHART_V1]

// [BEGIN BLOCK:TF_CONTROL_ATTACH_RIGHT_V1]
(function TF_CONTROL_ATTACH_RIGHT_V1(){
  'use strict';
  const right = document.getElementById('ip-right');
  if(!right) return;
  // если есть старая кнопка или select — переместить в верх правой колонки
  const tfBtn = document.getElementById('tfSwitch') || document.getElementById('tfSelect');
  if(tfBtn && tfBtn.parentElement !== right){
    tfBtn.style.margin = '0 0 8px 0';
    right.insertBefore(tfBtn, right.firstChild);
  }
})();
// [END BLOCK:TF_CONTROL_ATTACH_RIGHT_V1]

// [BEGIN BLOCK:PCS8_SELFAUDIT_V7]
(function PCS8_SELFAUDIT_V7(){
  const check=(t)=>{
    const p=document.getElementById('InfoPanel');
    const g=document.getElementById('graphs');
    const L=document.getElementById('ip-left');
    const R=document.getElementById('ip-right');
    if(!p||!g||!L||!R||g.parentElement!==R) throw new Error('[PCS-8][STOP] UI invariant broken');
    console.log('[PCS8][SELF_AUDIT t='+t+'] PASS');
  };
  setTimeout(()=>check(0),0);
  setTimeout(()=>check(250),250);
})();
// [END BLOCK:PCS8_SELFAUDIT_V7]
...
(function MTC_LAYOUT_V10(){
 ...
 if(!graphs.parentElement){
  panel.appendChild(graphs);
 }
 Object.assign(graphs.style,{
   width:'100%',height:'360px',
   margin:'0',borderTop:'1px solid #444',
   overflow:'hidden'
 });
