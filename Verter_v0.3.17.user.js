/* Legacy TM header preserved for reference
// ==UserScript==
// @name         Verter_v0.3.16
// @version      0.3.16
// @description  v0.3.14 + Cycles History capture & InfoPanel rendering (no changes to stable modules)
...
// @name        Verter_v0.3.17
// @version     0.3.17
// @description v0.3.16 + FAST5S (S5 candles + mode switch + decision context); stable modules unchanged
// @match        https://pocketoption.com/*
// @run-at       document-idle
// ==/UserScript==

// [SSOT:APP:BEGIN]
const BUILD_NAME = 'Verter_v0.3.14';
const VER = '0.3.17';
const APP = (function(){
...
// [BEGIN BLOCK:Fast5S:ModeSwitch]
let __fastMode = 'FAST5S';
function getActiveMode(){ return __fastMode; }
function setActiveMode(name){ __fastMode = (name === 'M1' ? 'M1' : 'FAST5S'); }
// [END BLOCK:Fast5S:ModeSwitch]

try{
  const __origSetActiveMode = setActiveMode;
  setActiveMode = function(name){
    __origSetActiveMode(name);
    try{ if(typeof DockBox!=='undefined' && DockBox?.requestRender){ DockBox.requestRender(); } }catch(e){}
  };
}catch(e){}
...
(function () {
  ...
  // [BEGIN BLOCK:CyclesHistory:Store]
  const CyclesHistory = (()=> {
    const RING = []; const CAP = 50;
    let lastClosedTradeId = null;
    return {
      push(rec){ RING.push(rec); if (RING.length > CAP) RING.shift(); },
      list(n=30){ return RING.slice(-n); },
      getLastId(){ return lastClosedTradeId; },
      setLastId(id){ lastClosedTradeId = id; }
    };
  })();
  // [END BLOCK:CyclesHistory:Store]
  ...
  var SessionStats=(function(){
    ...
    function onClose(type, pnl, finalBal, amount){
      ...
      if(s.startBalance!=null && s.lastBalance!=null) s.profitTotal=r2(s.lastBalance-s.startBalance);
      // [BEGIN BLOCK:CyclesHistory:CaptureOnClose]
      try{
        // 1) Дедуп по ID закрытой сделки
        const tid = (typeof TradeRowObserver!=='undefined' && TradeRowObserver && TradeRowObserver.lastClosedId)
          ? TradeRowObserver.lastClosedId
          : `${Date.now()}_${amount||0}`;
        if (CyclesHistory.getLastId() === tid) { /* уже фиксировали */ }
        else {
          CyclesHistory.setLastId(tid);

          // 2) Условия завершения цикла
          const ended =
            type === 'WIN' ||
            (typeof CycleAcc!=='undefined' && CycleAcc && CycleAcc.lossStreak >= 3) ||
            (typeof limitLoss!=='undefined' && typeof SessionStats!=='undefined' && SessionStats && SessionStats.lossesSum >= limitLoss) ||
            (typeof CycleAcc!=='undefined' && CycleAcc && typeof CycleAcc.maxTrades === 'number' && CycleAcc.trades >= CycleAcc.maxTrades);

          if (ended){
            // 3) Текущий баланс и PnL
            const curBal =
              (typeof BalanceStable!=='undefined' && BalanceStable && typeof BalanceStable.getStable==='function')
                ? BalanceStable.getStable()
                : (typeof Balance!=='undefined' && Balance && typeof Balance.read==='function' ? Balance.read() : null);

            const rec = {
              id: (typeof CycleAcc!=='undefined' && CycleAcc && typeof CycleAcc.id!=='undefined') ? CycleAcc.id : 0,
              end_ts: Date.now(),
              steps: (CycleAcc && CycleAcc.step) || 0,
              trades: (CycleAcc && CycleAcc.trades) || 0,
              won: (CycleAcc && CycleAcc.won) || 0,
              lost: (CycleAcc && CycleAcc.lost) || 0,
              wager: (CycleAcc && CycleAcc.wagerSum) || (amount || 0),
              pnl: (curBal!=null && CycleAcc && typeof CycleAcc.startBalance!=='undefined')
                    ? +(curBal - CycleAcc.startBalance).toFixed(2) : null,
              result: type // 'WIN'|'LOSS'|'RET'
            };

            // 4) Запись и отрисовка
            CyclesHistory.push(rec);
            if (typeof InfoPanel!=='undefined' && InfoPanel.updateCycles){
              InfoPanel.updateCycles(CyclesHistory.list(12));
            }
            try{ console.log('[CyclesHistory] saved id='+rec.id+' res='+rec.result+' steps='+rec.steps+' trades='+rec.trades+' pnl='+rec.pnl); }catch(_logErr){}
          }
        }
      }catch(e){ console.warn('[CyclesHistory] capture error', e); }
      // [END BLOCK:CyclesHistory:CaptureOnClose]
    }
    function snapshot(){
...
// [BEGIN BLOCK:Fast5S:Candles]
const CandleS5 = { cur: null, candles: [] };
function onTickS5(price, ts){
  const c = CandleS5.cur;
  if (!c || (ts - c.ts) >= 5000){
    const n = { ts, open: price, high: price, low: price, close: price };
    CandleS5.cur = n;
    CandleS5.candles.push(n);
    if (CandleS5.candles.length > 600) CandleS5.candles.shift();
  } else {
...
// [END BLOCK:Fast5S:Candles]

// [BEGIN BLOCK:Fast5S:TickWire]
try {
  tickSource.subscribe(function onTickForS5(p){
    onTickS5(p, Date.now());
  }, 100); // период опроса ≥100 мс
} catch(e){}
// [END BLOCK:Fast5S:TickWire]

try{ globalThis.candlesM1 = CandleState.candlesM1; }catch(e){}
try{ globalThis.CandleS5 = CandleS5; }catch(e){}
const Fast5SControl = { lastCandleTs: 0, lastDecisionTs: 0 };
function getCurrentFast5sCandle(){
  if (CandleS5.cur) return CandleS5.cur;
  const arr = CandleS5.candles;
  return arr.length ? arr[arr.length-1] : null;
}
function checkFast5SWindow(now){
  const candle = getCurrentFast5sCandle();
  if(!candle) return { ok:false, reason:'no-candle' };
  if(Fast5SControl.lastCandleTs === candle.ts) return { ok:false, reason:'duplicate', candle };
  const age = now - candle.ts;
  if(!isFinite(age)) return { ok:false, reason:'bad-age', candle };
  if(age < 500) return { ok:false, reason:'early', age, candle };
  if(age > 1200) return { ok:false, reason:'late', age, candle };
  return { ok:true, age, candle };
}
function markFast5sDecision(candle){
  if(candle && candle.ts){ Fast5SControl.lastCandleTs = candle.ts; }
  Fast5SControl.lastDecisionTs = Date.now();
}
...
`);
  ...
  var DockBox=(function(){
    ...
    payoutInput.addEventListener('change', function(e){
      ...
      try{
        if(globalThis.CFG && globalThis.CFG.thresholds){
          globalThis.CFG.thresholds.payoutMin=next;
        }
      }catch(err){}
    ...
    function renderHeader(force){
      ...
      left.innerHTML =
        ...
        ' | Frame: ' + ((typeof getActiveMode==='function')?getActiveMode():'M1');
      ...
      var decisionLine='Decision: <span class="v-dec '+decCls+'">'+decTxt+'</span>';
      var payoutSuffix='';
      if(meta.payout!=null && meta.payoutThr!=null){
        var ratio=meta.payout/100;
        var thr=meta.payoutThr;
      ...
      var skipsLine='Skips: payout '+(skips.payout||0)+' | low_vol '+(skips.low_vol||0)+' | conflict '+(skips.conflict||0)+' | cooldown '+(skips.cooldown||0)+' | fast5s '+(skips.fast5s||0);
...
var dockBoxFooter=(function(){
  try{
    const box=document.getElementById('DockBox');
    if(!box) return null;
    let footer=box.querySelector('.verter-footer');
    if(!footer){
      footer=document.createElement('div');
      footer.className='verter-footer';
      footer.style.cssText='padding:6px 8px;border-top:1px solid #222;display:flex;gap:8px;flex-wrap:wrap;';
      box.appendChild(footer);
    }
    return footer;
  }catch(e){ return null; }
})();
try{ globalThis.dockBoxFooter = dockBoxFooter; }catch(e){}

// [BEGIN BLOCK:Fast5S:UI]
(function addFastModeButtons(){
  try{
    const btnFast = document.createElement('button');
    btnFast.textContent = 'FAST5S';
    btnFast.addEventListener('click', ()=> setActiveMode('FAST5S'));
    const btnM1 = document.createElement('button');
    btnM1.textContent = 'M1';
    btnM1.addEventListener('click', ()=> setActiveMode('M1'));
    if (typeof dockBoxFooter!=='undefined' && dockBoxFooter) {
      dockBoxFooter.append(btnFast, btnM1);
    }
  }catch(e){}
})();
// [END BLOCK:Fast5S:UI]

  // [BEGIN BLOCK:CyclesHistory:PanelGlue]
  if (typeof InfoPanel !== 'undefined'){
    function ensureCyclesNode(){
      let el = document.getElementById('ip-cycles');
      if (!el && InfoPanel.root){
        el = document.createElement('div');
        el.id = 'ip-cycles';
        el.textContent = '—';
        InfoPanel.root.appendChild(el);
      }
      return el;
    }
    InfoPanel.updateCycles = function(list){
      const el = ensureCyclesNode(); if (!el) return;
      const data = list || [];
      if (!data.length){ el.textContent = '—'; return; }
      el.textContent = data
        .map(r => `#${r.id} ${r.result} s:${r.steps} t:${r.trades} pnl:${r.pnl ?? '?'}`)
        .join(' | ');
    };
  }
  // [END BLOCK:CyclesHistory:PanelGlue]

// [BEGIN BLOCK:Fast5S:DecisionContext]
function getDecisionCandles(){
  if (getActiveMode() === 'FAST5S'){
    return CandleS5.candles;
  } else {
    return (typeof candlesM1!=='undefined' && candlesM1) ? candlesM1 : (M1 && M1.candles);
  }
}
// Все места, где логика сигналов берёт свечи, должны использовать getDecisionCandles().
// Экспорт Signals НЕ менять.
// [END BLOCK:Fast5S:DecisionContext]
  ...
  var SignalEngine=(function(){
    ...
    function updateIndicators(){
      ...
      var appendedCurrent=false;
      if(st.cur){
        candles=candles.concat([{O:st.cur.O,H:st.cur.H,L:st.cur.L,C:st.cur.lastP}]);
        appendedCurrent=true;
      }
      var activeMode=(typeof getActiveMode==='function')?getActiveMode():'M1';
      var decisionCandles=(typeof getDecisionCandles==='function')?getDecisionCandles():null;
      if(Array.isArray(decisionCandles) && decisionCandles.length){
        var normalized=decisionCandles.map(function(c){
          if(!c) return null;
          if(c.O!=null) return {O:c.O,H:c.H,L:c.L,C:c.C};
          return {O:c.open,H:c.high,L:c.low,C:c.close};
        }).filter(Boolean);
        if(normalized.length){
          candles=normalized.slice(-260);
          appendedCurrent=false;
        }
      }
      if(activeMode==='FAST5S'){
        var fastCur=(CandleS5 && CandleS5.cur)?{O:CandleS5.cur.open,H:CandleS5.cur.high,L:CandleS5.cur.low,C:CandleS5.cur.close,lastP:CandleS5.cur.close}:null;
        if(fastCur){ candles=candles.concat([fastCur]); }
      } else if(st.cur && !appendedCurrent){
        candles=candles.concat([{O:st.cur.O,H:st.cur.H,L:st.cur.L,C:st.cur.lastP}]);
        appendedCurrent=true;
      }
  ...
  const DecisionState=(function(){
    ...
    base.skips=Object.assign({payout:0,low_vol:0,cooldown:0,conflict:0,fast5s:0}, base.skips||{});
  ...
  var DecisionEngine=(function(){
    ...
    const state={
      lastDecision:null,
      lastConflict:null,
      skips:{payout:0, low_vol:0, cooldown:0, conflict:0, fast5s:0},
    ...
    function updateFilters(filters){
      ...
      try{ DockBox.updateDecision({ filters, lastPick:DecisionState.lastPick }); }catch(e){}
    }
    ...
    function buildMeta(ctx){
      ...
      return {
        payout: ctx.payout,
        payoutThr: CFG.thresholds.payoutMin,
        atr14: ctx.atr14,
        atrMedian20: ctx.atrMedian20,
        cooldownSec: ctx.secsSinceLastTrade,
        cooldownThr: CFG.thresholds.cooldownSec
      };
    }
    ...
    function noteDecision(decision, ctx){
      ...
      try{
        DockBox.updateDecision({ last: displayStr, lastPick: DecisionState.lastPick, filters: filtersData, meta });
      }catch(e){}
    }
  ...
  (function(){
    ...
    setInterval(function(){
      ...
      var ctx=buildCtx();
      var activeMode=(ctx && ctx.mode)?ctx.mode:((typeof getActiveMode==='function')?getActiveMode():'M1');
      var fastGateInfo=null;
      ...
      if(!ctx.filters || ctx.filters.payout!==true){
        ...
        console.log('[SKIP] reason=payout payout=%s thr=%s', fmt2(ctx.payout), fmt2(DecisionEngine.CFG.thresholds.payoutMin*100), 'mode='+activeMode);
        return;
      }
      if(ctx.volOk!==true){
        DecisionEngine.noteSkip('low_vol');
        DecisionEngine.noteDecision(null, ctx);
        console.log('[SKIP] reason=low_vol atr=%s median=%s', fmt2(ctx.atr14), fmt2(ctx.atrMedian20), 'mode='+activeMode);
        return;
      }
      if(!ctx.filters.cooldown){
        DecisionEngine.noteSkip('cooldown');
        DecisionEngine.noteDecision(null, ctx);
        console.log('[SKIP] reason=cooldown secs=%s', ctx.secsSinceLastTrade, 'mode='+activeMode);
        return;
      }
      ...
      if(!best){
        ...
        if(conflict){
          ...
          console.log('[SKIP] reason=conflict top=%s rival=%s Δ=%s', conflict.top.name, conflict.rival.name, fmt2(conflict.delta), 'mode='+activeMode);
        }
        return;
      }

      if(activeMode==='FAST5S'){
        fastGateInfo=checkFast5SWindow(Date.now());
        if(!fastGateInfo || fastGateInfo.ok!==true){
          DecisionEngine.noteSkip('fast5s');
          DecisionEngine.noteDecision(null, ctx);
          var fastInfo = fastGateInfo || { reason:'no-candle' };
          var extraInfo = fastInfo.age!=null ? (' age='+fastInfo.age) : '';
          console.log('[FILTER skip:fast5s] reason='+fastInfo.reason+extraInfo, 'mode='+activeMode);
          return;
        }
      }
      ...
      if(best){
        ...
        try{ DockBox.updateDecision({ lastPick:DecisionState.lastPick }); }catch(err){}
      }
      ...
      if(typeof MARTIN_ENABLED==='undefined' || !MARTIN_ENABLED){
        ...
        if(payout!=null && payout<payoutThr){
          console.log('[OPEN][SKIP] reason=payout<thr sig=%s step=%d payout=%s', sig, pendingStepForLog, payoutFmt, 'mode='+activeMode);
          TradeGate.busy=false; TradeGate.tOpen=0; DockBox.setMode('IDLE');
          pendingStepForLog=null;
          return;
        }
        ...
        if(!sent){
          console.log('[OPEN][SKIP] reason=hotkey-false sig=%s step=%d payout=%s', sig, pendingStepForLog, payoutFmt, 'mode='+activeMode);
          TradeGate.busy=false; TradeGate.tOpen=0; DockBox.setMode('IDLE');
          pendingStepForLog=null;
          return;
        }
        ...
        if(activeMode==='FAST5S' && fastGateInfo && fastGateInfo.candle){ markFast5sDecision(fastGateInfo.candle); }
        console.log('[TRADE-OPEN] sig=%s side=%s step=%d bet=%s payout=%s', sig, side, pendingStepForLog, fmt2(amount), payoutFmt, 'mode='+activeMode);
      } else {
        var betRec = null;
        var betVal = null;
        try{
        ...
        if(payout!=null && payout<payoutThr){
          console.log('[OPEN][SKIP] reason=payout<thr sig=%s step=%d payout=%s', sig, stepForLog, payoutFmt, 'mode='+activeMode);
          TradeGate.busy=false; TradeGate.tOpen=0; DockBox.setMode('IDLE');
          pendingStepForLog=null;
          return;
        }
        ...
        if(!ok){
          console.log('[OPEN][SKIP] reason=smartBet-false sig=%s step=%d', sig, stepForLog, 'mode='+activeMode);
          TradeGate.busy=false; TradeGate.tOpen=0; DockBox.setMode('IDLE');
          pendingStepForLog=null;
          return;
        }
        ...
        DecisionEngine.noteTradeOpen();
        if(activeMode==='FAST5S' && fastGateInfo && fastGateInfo.candle){ markFast5sDecision(fastGateInfo.candle); }
        console.log('[TRADE-OPEN] sig=%s side=%s step=%d bet=%s payout=%s', sig, side, stepForLog, fmt2(betVal), payoutFmt, 'mode='+activeMode);
      }
    }, 1000);
...
/*
Build from base v0.3.14
Feature: Cycles History (capture on onClose + InfoPanel render)
Unchanged: stable modules (TradeRowObserver, CandleChart, SessionStats core, InfoPanel core, PauseCtl, BalanceStable, DockBox, payout UI)
No duplicate globals, syntax OK
*/

/*
[BuildReport]
base_path=/workspace/NewVerter/Verter_v0.3.16.user.js
base_sha256=70e27abce2c05800c0ade4aa7c16798f85faab9f551a8cb0f60d5ca7e221c533 OK
version=0.3.17
blocks=Fast5S:Candles,Fast5S:TickWire,Fast5S:ModeSwitch,Fast5S:DecisionContext,Fast5S:UI
uniq: Signals=1 MARTIN_ENABLED=1 betArray1=1 SessionStats=1 TradeRowObserver=1 APP=1 VER=1 BUILD_NAME=1
markers: paired OK
readonly: OK
syntax: OK
*/
