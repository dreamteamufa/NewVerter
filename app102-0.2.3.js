...
// @version      0.2.3
// @description  Fix disappearing InfoPanel: drag v2, watchdog v2, disable duplicate UI canvas (PCS-8 v3.2)
...
/** Квантизированные бары от начала минуты. TF: 5s,10s,15s,30s,1m,5m */
const MTC = (() => {
  ...
  function quantStart(now, period){
    const m0 = Math.floor(now/60000)*60000;
    return now - ((now - m0) % period);
  }
  function seedIfNeeded(now, price){
    if (seeded || !Number.isFinite(price)) return;
    for (const [lbl,period] of Object.entries(TF)){
      const t0 = quantStart(now, period);
      cur[lbl] = { t:t0, O:price, H:price, L:price, C:price };
    }
    seeded = true;
  }
  function onTick(now, price){
    ...
    let closedAny = false;
    for (const [lbl,period] of Object.entries(TF)){
      const slot = quantStart(now, period);
      let b = cur[lbl];
      if (slot > b.t){
        frames[lbl].push({ t: b.t + period, O:b.O, H:b.H, L:b.L, C:b.C });
    ...
    try { MTC.drawCandles && MTC.drawCandles(); } catch(_){}
    if (closedAny){
      const m1 = frames['1m']; if (m1.length){
        const t = new Date(m1[m1.length-1].t); const ok = t.getUTCSeconds()===0;
        console.log('[MTC][1m-close]', t.toISOString(), 'minuteAligned=', ok);
      }
      for (const lbl of Object.keys(TF)){
        const arr = frames[lbl]; if (!arr.length) continue;
        const c = arr[arr.length-1];
        console.log(`[CANDLE][${lbl}] O=${c.O} H=${c.H} L=${c.L} C=${c.C} t=${new Date(c.t).toISOString()}`);
      }
    }
  }
...
let queryPrice = () => {
    ...
    if (!window.__MTC_seedBooted) {
        window.__MTC_seedBooted = true;
// [BEGIN BLOCK:MTC_SYNC_V9]
(function MTC_SYNC_V9(){
 'use strict';
 ...
 MTC.startMinuteSeed = function(cb){
  ...
  console.log('[MTC][SYNC] minuteAligned=true (SeedDelay=',delay,')');
 };
 MTC.startMinuteSeed(()=>MTC.drawCandles?.());
})();
// [END BLOCK:MTC_SYNC_V9];
    }
    const BalanceStable = Number.isFinite(globalPrice);
    // [BEGIN BLOCK:CANDLE_BUILDER_V2_HOOK]
    if (BalanceStable) { try { MTC.onTick(Date.now(), globalPrice); } catch(_) {} }
...
function addUI() {
    ...
    if (!panel) {
        ...
        makeLine(`Start Balance: $${startBalance}.  Start Time: ${startTime}`);
        makeLine(`MODE: ${mode}`);
        makeLine(`Trading symbol:<span id="trading-symbol"> ${symbolName}</span>.`);
        makeLine(`Reverse: ${reverse}.`);
        makeLine(`Cycle profit: $<span id="profit">0</span> Won: <span id="won-percent">0</span>%  Wager: $ <span id="wager">0</span>`);
        makeLine(`Current time: <span id="time">0:00</span>`);
        makeLine(`% profit on current pair: <span id="profit-percent">0</span> %`);
        makeLine(`Indicator: <span id="sqz-indicator">flat</span>Trend: <span id="price-trend">flat</span>globalTrend: <span id="global-trend">flat</span>`);
        makeLine(`Trade Direction: <span id="trade-dir">flat</span>`);
        makeLine(`Max Step in Cycle: <span id="max-step">0</span> / Total Profit: <span id="total-profit">0</span> USD`);
        makeLine(`Cycles history:<br><span id="cycles-history"></span>`);
    }
...
if (typeof window !== 'undefined') window.__MTC_PANEL_DRAG_V2 = true;
addUI();

// [BEGIN BLOCK:PANEL_WATCHDOG_V1]
if(false){
(function PANEL_WATCHDOG_V1(){
 'use strict';
 const panel=document.getElementById('InfoPanel');
 const graphs=document.getElementById('graphs');
 if(!panel){console.error('[WATCHDOG][ERR] No InfoPanel');return;}
 const fix=()=>{
   if(!document.body.contains(panel)) document.body.appendChild(panel);
   if(graphs && graphs.parentElement!==panel) panel.appendChild(graphs);
 };
 new MutationObserver(fix).observe(document.body,{childList:true,subtree:true});
 window.addEventListener('focus',fix);
 window.addEventListener('resize',fix);
 console.log('[WATCHDOG][PASS] InfoPanel persistence active');
})();
}
// [END BLOCK:PANEL_WATCHDOG_V1];

// [BEGIN BLOCK:PANEL_WATCHDOG_V2]
(function PANEL_WATCHDOG_V2(){
  'use strict';
  const fix=()=>{
    const p=document.getElementById('InfoPanel');
    if(!p){ console.warn('[WATCHDOG] re-create InfoPanel'); return; }
    let g=document.getElementById('graphs');
    if(!g){
      g=document.createElement('div'); g.id='graphs'; p.appendChild(g);
    } else if(g.parentElement!==p){
      p.appendChild(g);
    }
  };
  new MutationObserver(fix).observe(document.body,{childList:true,subtree:true});
  ...
  fix();
  console.log('[WATCHDOG][PASS] v2');
})();
// [END BLOCK:PANEL_WATCHDOG_V2]

// [BEGIN BLOCK:PANEL_DRAG_V1]
if(false){
(function PANEL_DRAG_V1(){
 'use strict';
 const panel=document.getElementById('InfoPanel');
 if(!panel) return console.error('[PANEL][ERR] missing InfoPanel');
 panel.style.position='fixed';
 const drag=document.createElement('div');
 Object.assign(drag.style,{
  height:'18px',background:'#222',cursor:'grab',userSelect:'none'
 });
 drag.textContent='⇅';
 panel.insertBefore(drag,panel.firstChild);
 let ox=0,oy=0,dragging=false;
 const save=()=>localStorage.__PANEL_POS=JSON.stringify({x:panel.offsetLeft,y:panel.offsetTop});
 drag.onmousedown=e=>{dragging=true;panel.style.right='auto';panel.style.bottom='auto';ox=e.clientX-panel.offsetLeft;oy=e.clientY-panel.offsetTop;drag.style.cursor='grabbing';};
 window.onmouseup=()=>{if(dragging){dragging=false;save();drag.style.cursor='grab';}};
 window.onmousemove=e=>{
  if(!dragging) return;
  panel.style.left=Math.max(0,e.clientX-ox)+'px';
  panel.style.top=Math.max(0,e.clientY-oy)+'px';
 };
 const pos=localStorage.__PANEL_POS?JSON.parse(localStorage.__PANEL_POS):null;
 if(pos){panel.style.right='auto';panel.style.bottom='auto';panel.style.left=pos.x+'px';panel.style.top=pos.y+'px';}
 console.log('[PANEL][PASS] draggable enabled');
})();
}
// [END BLOCK:PANEL_DRAG_V1];

// [BEGIN BLOCK:PANEL_DRAG_V2]
(function PANEL_DRAG_V2(){
  'use strict';
  const p=document.getElementById('InfoPanel'); if(!p){console.error('[PANEL] no panel');return;}
  p.style.position='fixed';
  const drag=document.createElement('div');
  Object.assign(drag.style,{height:'18px',background:'#222',cursor:'grab',userSelect:'none'});
  drag.textContent='⇅';
  p.insertBefore(drag,p.firstChild);
  ...
  const onUp=()=>{
    ...
    localStorage.__PANEL_POS=JSON.stringify({x:p.offsetLeft,y:p.offsetTop});
  ...
  const pos=localStorage.__PANEL_POS && JSON.parse(localStorage.__PANEL_POS);
  if(pos){ p.style.right='auto'; p.style.bottom='auto'; p.style.left=pos.x+'px'; p.style.top=pos.y+'px'; }
  console.log('[PANEL][PASS] drag v2');
})();
// [END BLOCK:PANEL_DRAG_V2]

// [BEGIN BLOCK:MTC_LAYOUT_V9]
(function MTC_LAYOUT_V9(){
 'use strict';
 const panel=document.getElementById('InfoPanel');
 if(!panel) return console.error('[MTC][ERR] InfoPanel missing');
 let graphs=document.getElementById('graphs');
 if(!graphs){
  graphs=document.createElement('div');
  graphs.id='graphs';
 ...
 Object.assign(graphs.style,{
  ...
  height:'320px', margin:'0 auto', borderTop:'1px solid #444'
 });
 graphs.style.minHeight='240px';
 graphs.style.overflow='hidden';
})();
// [END BLOCK:MTC_LAYOUT_V9]
panel && panel.appendChild(document.getElementById('graphs'));
const signalGraph=document.getElementById('signalGraph');
if(signalGraph) signalGraph.style.display='none';

// [BEGIN BLOCK:MTC_RENDER_V9]
(function MTC_RENDER_V9(){
 'use strict';
 let last=0;
 function loop(t){
  if(t-last>200){ last=t; MTC.drawCandles?.(); }
  requestAnimationFrame(loop);
 }
 requestAnimationFrame(loop);
})();
// [END BLOCK:MTC_RENDER_V9]

// [BEGIN BLOCK:MTC_TF_SELECT_V1]
(function MTC_TF_SELECT_V1(){
 'use strict';
 if(!localStorage.__MTC_TF) localStorage.__MTC_TF='5s';
 const old=document.getElementById('tfSwitch');
 if(old) old.style.display='none';
 const sel=document.createElement('select');
 sel.id='tfSelect';
 const options=['5s','10s','15s','30s','1m','5m'];
 ...
 Object.assign(sel.style,{
  ...
  borderRadius:'6px',padding:'4px 6px',margin:'4px',cursor:'pointer'
 });
 sel.onchange=()=>{
  const v=sel.value;
  localStorage.__MTC_TF=v;
  window.__MTC_activeTF=v;
  console.log('[MTC][TF] set →',v);
  window.MTC?.drawCandles?.();
 };
 document.getElementById('InfoPanel')?.appendChild(sel);
 window.__MTC_activeTF=sel.value;
})();
// [END BLOCK:MTC_TF_SELECT_V1];

// [BEGIN BLOCK:PANEL_VERSION_V9]
(function PANEL_VERSION_V9(){
 'use strict';
 const panel=document.getElementById('InfoPanel');
 if(panel){
  const ver=document.createElement('div');
  ver.textContent='Сборка: app102 v0.2.3';
  ver.style.fontSize='12px';
  ver.style.color='#ccc';
  panel.appendChild(ver);
  console.log('[INFO] Panel version updated → app102 v0.2.3');
 }
})();
// [END BLOCK:PANEL_VERSION_V9]

// [BEGIN BLOCK:PCS8_SELFAUDIT_V3]
if(false){
(function PCS8_SELFAUDIT_V3(){
 const p=document.getElementById('InfoPanel');
 const g=document.getElementById('graphs');
 if(!p||!g){
   console.warn('[PCS-8][WARN] Panel not yet ready');
   return;
 }
 if(g.parentElement!==p){
   console.warn('[PCS-8][FIX] Reattaching graphs');
   p.appendChild(g);
 }
 if(getComputedStyle(p).position!=='fixed'){
   console.warn('[PCS-8][FIX] Forcing fixed position');
   p.style.position='fixed';
 }
 console.log('[PCS-8][PASS][SelfAudit V3]');
})();
}
// [END BLOCK:PCS8_SELFAUDIT_V3];

// [BEGIN BLOCK:PCS8_SELFAUDIT_V4]
(function PCS8_SELFAUDIT_V4(){
  const ok=()=>{
    const p=document.getElementById('InfoPanel');
    const g=document.getElementById('graphs');
    if(!p||!g) return false;
    if(g.parentElement!==p) { p.appendChild(g); }
    if(getComputedStyle(p).position!=='fixed') p.style.position='fixed';
    return true;
  };
  if(!ok()){
    console.warn('[PCS-8][WARN] panel not ready, retrying...');
    setTimeout(()=>{ if(!ok()) console.error('[PCS-8][WARN] still not ready'); else console.log('[PCS-8][PASS][SelfAudit V4]'); }, 250);
    return;
  }
  console.log('[PCS-8][PASS][SelfAudit V4]');
})();
// [END BLOCK:PCS8_SELFAUDIT_V4]
...
(function setupSmoothChart(){
  ...
  const logicalW = 640, logicalH = 240;
  function resizeHiDPI(){
    const dpr = Math.max(1, Math.floor(window.devicePixelRatio || 1));
    cv.width = logicalW * dpr; cv.height = logicalH * dpr;
    cv.style.width = '100%'; cv.style.maxWidth = logicalW+'px'; cv.style.height = logicalH+'px';
    const ctx = cv.getContext('2d'); ctx.setTransform(dpr,0,0,dpr,0,0);
    return ctx;
  }
  let ctx = resizeHiDPI();
  window.addEventListener('resize', () => { ctx = resizeHiDPI(); });

  function draw(){
    const tf = localStorage.__MTC_TF || '5s';
    ...
    for(let i=1;i<=3;i++){ const y=Math.round(i*logicalH/4); ctx.moveTo(0,y); ctx.lineTo(logicalW,y); }
    ctx.stroke(); ctx.globalAlpha=1;

    const L=8,R=8,T=6,B=8, innerW=logicalW-L-R, innerH=logicalH-T-B;
    const xStep= innerW/Math.max(1,hist.length);
    const y = v => T + (hi - v) * (innerH/(hi-lo));

    for(let i=0;i<hist.length;i++){
      const c=hist[i]; const x=L + i*xStep + xStep*0.5;
      ctx.beginPath(); ctx.moveTo(x, y(c.H)); ctx.lineTo(x, y(c.L));
      ctx.lineWidth=1; ctx.strokeStyle='#9aa4b2'; ctx.stroke();
      const up = c.C >= c.O, bw = Math.max(1, Math.floor(xStep*0.6));
      const x0 = Math.round(x - bw/2), y0 = Math.round(y(Math.max(c.O,c.C)));
      const h  = Math.max(1, Math.abs(Math.round(y(c.O)-y(c.C))));
      ctx.fillStyle = up ? '#2ecc71' : '#e74c3c'; ctx.fillRect(x0, y0, bw, h);
    }
  }

  MTC.drawCandles = draw;
  draw();
})();
...
const CandleChartUI = (() => {
  (function(){ return; /* UI disabled: RAF owns canvas */ })();
  const __CANDLE_UI_INTERVAL_ID = undefined;
  // [BEGIN BLOCK:DISABLE_OLD_DRAW_TIMER]
  try { if (typeof __CANDLE_UI_INTERVAL_ID !== 'undefined') clearInterval(__CANDLE_UI_INTERVAL_ID); } catch(_){ }
  if (false){ /* setInterval(draw,1000) отключён. Рендер через RAF. */ }
