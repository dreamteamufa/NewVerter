...
// @version     0.1.6
// @description MTC-in-panel only; TF control fixed; minute-quant; single host; RAF only
...
const MTC = (() => {
  ...
  function onTick(now, price){
    ...
    if (closed){
      ...
      const t = new Date(MTC.frames['1m'].slice(-1)[0]?.t || 0);
      const ok = t && t.getUTCSeconds()===0;
...
function addUI() {
    ...
    graphContainer.style.maxWidth = '640px';
...
const CandleChartUI = (() => {
  function createPanel(){
    ...
    if (!host) return;
    let box = document.getElementById('candle-chart-box');
    if (!box){
    ...
    let canvas = document.getElementById('candle-chart-canvas');
    if (!canvas){
      canvas = document.createElement('canvas');
      canvas.id = 'candle-chart-canvas';
      canvas.width = 640;
      canvas.height = 240;
      canvas.style.cssText = 'display:block;background:#0b0e13;border:1px solid #1f2630;border-radius:8px;';
  ...
  function draw(){
    ...
    const xStep=cv.width/Math.max(1,data.length);
    data.forEach((c,i)=>{
...
(function addTfButton(){
  const host = document.getElementById('candle-chart-box') || document.getElementById('graphs');
  if (!host) return;
  ...
  function updateLabel(){
    btn.textContent = 'TF: ' + window.__MTC_activeTF;
    const sel = document.querySelector('#candle-chart-box select');
    if (sel) { sel.value = window.__MTC_activeTF; }
  }
  ...
  if (sel) sel.onchange = e => {
    window.__MTC_activeTF = e.target.value;
    updateLabel();
    window.__MTC_scheduleDraw && window.__MTC_scheduleDraw(false);
  };
...
// BuildReport
// [CODEX][OK] single graphs host in panel
// [CODEX][OK] candle UI bound to MTC only
// [CODEX][OK] TF button drives active TF
// [CODEX][OK] RAF-only renderer active
// [CODEX][OK] minute-quant verified
