// ==UserScript==
// @name         Verter_v0.3.3
// @description  Auto-trade + Signals (EMA9/15, SQZ) + CLOSE via legacy Trades observer (guard+dedupe) + BalanceStable(2s) + DockBox (+50%) + Stats (Wager on DOM close) + One-At-A-Time Gate + PCS-8 v3.1 (Colon-Zero Lock)
// @version      0.3.3
...
// [SSOT:APP:BEGIN]
const BUILD_NAME = 'Verter_v0.3.3';
const VER = '0.3.3';
...
(function () {
  ...
  // [BEGIN MOD:Candles]
  const CandleState={ticks:[],candlesM1:[],cur:null};
  ...
  var DockBox=(function(){
    ...
    function renderHeader(){
      ...
      function pct(val){ return val==null || !isFinite(val) ? '—' : val.toFixed(1)+'%'; }
      function share(stat){ return totalOpens>0 && stat.open>0 ? pct(stat.open/totalOpens*100) : (totalOpens>0?pct(0):'—'); }
      function wr(stat){ var denom=(stat.win||0)+(stat.loss||0); return denom>0?pct(stat.win/denom*100):'—'; }
      ...
      right.innerHTML = sigLine + '<br>' + metricsLine1 + '<br>' + metricsLine2;
    }
    ...
    return { stop, getState:function(){return Candles.getState();}, setMode, setSigReady, incSigHit };
  })();
  ...
  var SignalEngine=(function(){
    ...
    function evalAll(){
      ...
      var okEMA915=null, sqz=null;
      if(closes.length>=15){
        var eF=ema(closes,9), eS=ema(closes,15);
        if(eF[eF.length-1]>eS[eS.length-1]) okEMA915={side:'BUY',sig:'EMA9/15'};
        else if(eF[eF.length-1]<eS[eS.length-1]) okEMA915={side:'SELL',sig:'EMA9/15'};
      }
      if(closes.length>=20){
  ...
  var TradeBridge=(function(){
    var pending=null; // {side, amount, tOpen, pre, sig}
    function sigKeyOf(sig){ if(sig==='EMA9/15') return 'EMA915'; if(sig==='SQZ') return 'SQZ'; return null; }
    function arm(side, amount, meta){
      ...
      Object.defineProperty(rec,'won',{
        set:function(v){
          ...
          if(sigKey && SigStats && SigStats[sigKey]){
      ...
      pending = { side: side, amount: amount, step: pendingStepForLog, tOpen: now(), pre: BalanceStable.getStable()||Balance.read()||null, sig: sigMeta };
      var key = sigKeyOf(sigMeta);
      if(key && SigStats && SigStats[key]){
        SigStats[key].open++;
        SigStats[key].opensAt.push(Date.now());
      }
  ...
  (function(){
    ...
    setInterval(function(){
      ...
      if(typeof MARTIN_ENABLED==='undefined' || !MARTIN_ENABLED){
        ...
        console.log('[TRADE-OPEN] sig='+sig+' side='+side+' step=0 bet='+fmt2(amount)+' payout='+(payout!=null?payout:'—'));
        TradeBridge.arm(side, amount, { sig });
        ...
        console.log('[TRADE-OPEN] sig='+sig+' side='+side+' step='+stepForLog+' bet='+fmt2(betVal)+' payout='+(payout!=null?payout:'—'));
        TradeBridge.arm(side, betVal, { sig });
