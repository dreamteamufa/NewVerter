...
// @version      0.2.1
// @description  Draggable InfoPanel + TF Select + PCS-8 v3.2 rev B compliance
...
function addUI() {
    ...
    (function makeDraggable(el){
      if (!el) return;
      if (window.__MTC_PANEL_DRAG_V2) return;
...
(function PANEL_VERSION_V9(){
 ...
 if(panel){
  ...
  ver.textContent='Сборка: app102 v0.2.1';
  ver.style.fontSize='12px';
  ver.style.color='#ccc';
  panel.appendChild(ver);
  console.log('[INFO] Panel version updated → app102 v0.2.1');
 }
})();
// [END BLOCK:PANEL_VERSION_V9]

// [BEGIN BLOCK:PCS8_SELFAUDIT_V2]
(function PCS8_SELFAUDIT_V2(){
 const p=document.getElementById('InfoPanel');
 const g=document.getElementById('graphs');
 const s=document.getElementById('tfSelect');
 if(!p||!g||g.parentElement!==p) throw new Error('[PCS-8][STOP] graphs not in InfoPanel');
 if(getComputedStyle(p).position!=='fixed') throw new Error('[PCS-8][STOP] InfoPanel not fixed');
 if(!s||s.value!==window.__MTC_activeTF) throw new Error('[PCS-8][STOP] TFSelect mismatch');
 console.log('[PCS-8][PASS][SelfAudit]');
})();
 // [END BLOCK:PCS8_SELFAUDIT_V2];
