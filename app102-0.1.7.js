...
// @version     0.1.7
// @description Minute-quant MTF candles; in-panel chart; RAF renderer; black TF button
...
const MTC = (() => {
  ...
  function onTick(now, price){
    ...
    try { window.__MTC_scheduleDraw && window.__MTC_scheduleDraw(closedAny); } catch(_){}
    if (closedAny){
...
let queryPrice = () => {
    ...
    if (Number.isFinite(globalPrice)) { try { MTC.onTick(Date.now(), globalPrice); } catch(_) {} }
...
// [BEGIN BLOCK:CHART_HOST_IN_PANEL]
(function ensureGraphsInPanel(){
  const panel = document.getElementById('InfoPanel') || document.body;
  ...
  Object.assign(graphs.style, {
    position:'relative', zIndex:'10000', width:'100%', maxWidth:'640px', height:'320px'
  });

  // удалить дубликаты #graphs, если вдруг есть
  const all = Array.from(document.querySelectorAll('#graphs'));
  for (const n of all.slice(1)) { try { n.remove(); } catch(_) {} }
})();
// [END BLOCK:CHART_HOST_IN_PANEL]
...
(function setupSmoothChart(){
  ...
  const ctx = resizeHiDPI(); window.addEventListener('resize', resizeHiDPI);

  let rafPending = false;
  window.__MTC_activeTF = window.__MTC_activeTF || '5s';
  window.__MTC_scheduleDraw = function(){ if(rafPending) return; rafPending=true; requestAnimationFrame(()=>{rafPending=false; draw();}); };
...
(function addTfButton(){
  ...
  if (!ctrl){
    ctrl = document.createElement('div'); ctrl.id='candle-chart-controls';
    ctrl.style.cssText='display:flex;gap:8px;align-items:center;margin:6px 0;';
    graphs.prepend(ctrl);
  }
  const btn = document.createElement('button');
  btn.id='tf-switcher'; btn.type='button';
  ...
  window.__MTC_activeTF = window.__MTC_activeTF || '5s';
  function label(){ btn.textContent = 'TF: ' + window.__MTC_activeTF; }
  btn.addEventListener('click', ()=>{ const i=order.indexOf(window.__MTC_activeTF); window.__MTC_activeTF=order[(i+1)%order.length]; label(); try{ window.__MTC_scheduleDraw(); }catch(_){}} );
  label(); ctrl.appendChild(btn);
})();
