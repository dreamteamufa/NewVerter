// ==UserScript==
// @name         Verter_v0.4.2
// @description  Auto-trade + Signals (EMA3/9, EMA9/15, SQZ) + CLOSE via legacy Trades observer (guard+dedupe) + BalanceStable(2s) + DockBox (+50%) + Stats (Wager on DOM close) + One-At-A-Time Gate + PCS-8 v3.1 (Colon-Zero Lock) + Cycles & Deals history (DockBox)
// @version      0.4.2
...
(function () {
  ...
  // [BEGIN BLOCK:DEALS:STATE]
  let vx_deals = [];                     // последние N исходов сделок
  const VX_DEALS_MAX = 30;
  let vx_cycleWinProfitSum = 0;          // суммарная прибыль WIN за цикл
  let vx_cycleLossSum = 0;               // суммарные потери LOSS за цикл
  let vx_cycleWins = 0, vx_cycleLosses = 0;

  function vx_pushDeal(type, amt, pnl){
    const tag = (type==='WIN'?'WIN':type==='LOSS'?'LOSS':'RET') + (amt!=null?(':'+((+amt)||0)):'');
    vx_deals.push(tag);
    if (vx_deals.length>VX_DEALS_MAX) vx_deals = vx_deals.slice(-VX_DEALS_MAX);
    if (type==='WIN'){
      const winProfit = (pnl!=null ? (+pnl) : 0);
      vx_cycleWinProfitSum += winProfit;
      vx_cycleWins++;
    } else if (type==='LOSS'){
      const loss = (amt!=null ? (+amt) : 0);
      vx_cycleLossSum += loss;
      vx_cycleLosses++;
    }
  }
  // /* [END BLOCK:DEALS:STATE] */
  ...
  // [BEGIN BLOCK:DEALS:HOOK]
  (function(){
    // перехват закрытия сделки
    try{
      const _ssOnClose = SessionStats.onClose;
      SessionStats.onClose = function(type, pnl, finalBal, amount){
        const r = _ssOnClose.apply(this, arguments);
        try{
          vx_pushDeal(type, amount, pnl);
          if (typeof DockBox!=='undefined' && DockBox.renderHeader)
            DockBox.renderHeader();
        }catch(e){}
        return r;
      };
    }catch(e){}

    // завершение цикла → обновить и обнулить накопители
    try{
    ...
    DockBox.renderHeader = function(){
      ...
      try{
        ...
        try{
          ...
          n = arr.length; last = arr[n-1] || null;
        }catch(e){}

        const id1='vx-cycles-hdr', id2='vx-deals-hdr';
        let e1 = host.querySelector('#'+id1);
        if (!e1){ e1=document.createElement('span'); e1.id=id1; e1.style.marginLeft='8px'; host.appendChild(e1); }
        let e2 = host.querySelector('#'+id2);
        if (!e2){ e2=document.createElement('span'); e2.id=id2; e2.style.marginLeft='8px'; host.appendChild(e2); }

        const status = last && (last.status||last.result||'');
        const pnl = last && ((last.profit??last.pnl)??0);
        const max = last && (last.maxStep??0);

        // текст: циклы + итоги последнего
        e1.textContent = `| Cycles: ${n}${last?` | Last: ${String(status).toUpperCase()} P=${pnl} S=${max}`:''}`;

        // текст: суммы и сделки
        const sums = `ΣWin=${vx_cycleWinProfitSum} | ΣLoss=${vx_cycleLossSum}`;
        e2.textContent = vx_deals.length
          ? `| ${sums} | Deals: ${vx_deals.join(', ')}`
          : `| ${sums} | Deals: —`;
      }catch(e){}
    };
  })();
  // /* [END BLOCK:DEALS:DOCKBOX] */
