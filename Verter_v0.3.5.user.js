// ==UserScript==
// @name         Verter_v0.3.5
// @description  Auto-trade + Signals (EMA9/15, SQZ) + BalanceStable + DockBox + Stats + One-At-A-Time Gate + PCS-8 v3.1 + Pause Control (UI + Auto)
// @version      0.3.5
...
// [SSOT:APP:BEGIN]
const BUILD_NAME = 'Verter_v0.3.5';
const VER = '0.3.5';
...
(function () {
  ...
  var DockBox=(function(){
    ...
    inpLoss.addEventListener('change', function(){
      PauseCtl.setAutoLosses(+inpLoss.value || 1);
      inpLoss.value = PauseCtl.cfg.autoOnLosses;
    });
    inpAutoMin.addEventListener('change', function(){
      PauseCtl.setDurationMin(+inpAutoMin.value || 1);
      inpAutoMin.value = PauseCtl.cfg.durationMin;
    });
    inpManualMin.addEventListener('change', function(){
      PauseCtl.setManualDurationMin(+inpManualMin.value || 1);
      inpManualMin.value = PauseCtl.cfg.manualDurationMin;
    });
