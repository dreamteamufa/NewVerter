// ==UserScript==
// @name         Verter_v0.3.2
// @description  Auto-trade + Signals (EMA3/9, EMA9/15, SQZ) + CLOSE via legacy Trades observer (guard+dedupe) + BalanceStable(2s) + DockBox (+50%) + Stats (Wager on DOM close) + One-At-A-Time Gate + PCS-8 v3.1 (Colon-Zero Lock)
// @version      0.3.2
...
// [SSOT:APP:BEGIN]
const BUILD_NAME = 'Verter_v0.3.2';
const VER = '0.3.2';
// [SSOT:APP:END]
...
(function () {
  ...
  var TradeBridge=(function(){
    ...
    function arm(side, amount){
      ...
      pending = { side: side, amount: amount, step: pendingStepForLog, tOpen: now(), pre: BalanceStable.getStable()||Balance.read()||null };
...
if(typeof MARTIN_ENABLED==='undefined' || !MARTIN_ENABLED){
  ...
  console.log('[TRADE-OPEN] step=0 bet='+fmt2(amount)+' side='+side);
  ...
  console.log('[TRADE-OPEN] step='+stepForLog+' bet='+fmt2(betVal)+' side='+side);
  TradeBridge.arm(side, betVal);
