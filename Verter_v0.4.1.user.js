// ==UserScript==
// @name         Verter_v0.4.1
// @description  Auto-trade + Signals (EMA3/9, EMA9/15, SQZ) + CLOSE via legacy Trades observer (guard+dedupe) + BalanceStable(2s) + DockBox (+50%) + Stats (Wager on DOM close) + One-At-A-Time Gate + PCS-8 v3.1 (Colon-Zero Lock) + Cycles history (built-in)
// @version      0.4.1
...
(function () {
  ...
  function vx_makeCycleTile(status, profit, maxStep, ts){
    ...
    el.textContent = `${String(status).toUpperCase()} • P=${profit} • S=${maxStep}`;
    return el;
  }
  ...
  function vx_cyclesUiBoot(){
    ...
    Object.assign(root.style,{position:'fixed',right:'8px',bottom:'8px',zIndex:'2147483647',background:'rgba(0,0,0,.6)',color:'#fff',padding:'8px',borderRadius:'6px',font:'12px/1.3 -apple-system,Segoe UI,Roboto,Arial',maxWidth:'300px'});
    root.innerHTML = `
      <div style="opacity:.9;margin-bottom:6px">Verter_v0.4.1 • Cycles</div>
      <div style="margin-bottom:6px">Max Step in Cycle: <span id="vx-maxstep">0</span></div>
      <div>Cycles history:</div>
      <div id="vx-cycles-history" style="display:flex;flex-direction:column;gap:4px;margin-top:4px;max-height:40vh;overflow:auto"></div>
      <style>
  ...
  function vx_renderCycleEnd(status){
    try{
      ...
      const pnl = (last.profit??last.pnl)??0;
      const max = last.maxStep??0;
      vx_cyclesHistoryDiv.prepend(vx_makeCycleTile(status, pnl, max, Date.now()));
...
if(typeof MARTIN_ENABLED==='undefined' || !MARTIN_ENABLED){
  ...
  (function(){
    ...
    try{
      const ss = (typeof SessionStats!=='undefined' && SessionStats && SessionStats.get && SessionStats.get()) || null;
      if (vx_cyclesHistoryDiv && ss && Array.isArray(ss.cycles) && ss.cycles.length){
        for (let i=ss.cycles.length-1;i>=0;i--){
          const c = ss.cycles[i];
          const pnl = (c.profit??c.pnl)??0;
          const max = c.maxStep??0;
          vx_cyclesHistoryDiv.append(vx_makeCycleTile(c.status||c.result||'', pnl, max, c.endedAt||Date.now()));
        }
      }
    }catch(e){}
    try{
    ...
    try{
      if (typeof DockBox!=='undefined' && DockBox && typeof DockBox.renderHeader!=='function'){
        DockBox.renderHeader = function(){};
      }
    }catch(e){}
    // обновление Max Step при закрытии сделки
    try{
      const _ssOnClose = SessionStats.onClose;
      SessionStats.onClose = function(type, ...rest){
        const r = _ssOnClose.apply(this, [type, ...rest]);
        try{
          const cur = (window.maxStepInCycle!==undefined ? window.maxStepInCycle : (window.currentBetStep||0));
          if (vx_maxStepLabel) vx_maxStepLabel.textContent = String(cur|0);
        }catch(e){}
        return r;
      };
    }catch(e){}
    // дорисовка плитки при завершении цикла
    try{
      const _cycleEnd = window.cycleEnd;
      window.cycleEnd = function(status){
        const r = _cycleEnd.apply(this, arguments);
        try{ vx_renderCycleEnd(status); }catch(e){}
        try{ if(typeof DockBox!=='undefined' && DockBox?.renderHeader){ DockBox.renderHeader(); } }catch(e){}
        return r;
      };
    }catch(e){}
    try{ if(typeof DockBox!=='undefined' && DockBox?.renderHeader){ DockBox.renderHeader(); } }catch(e){}
...
(function(){
  ...
  DockBox.renderHeader = function(){
    ...
    try{
      const host = document.querySelector('.verter-dock-header') || document.querySelector('[data-verter-header]');
      if (!host) return;
      let n = 0, last = null;
      try{
        const ss = (SessionStats && SessionStats.get && SessionStats.get()) || null;
        const arr = (ss && ss.cycles) || [];
        n = arr.length;
        last = arr[n-1] || null;
      }catch(e){}
      const id='vx-cycles-hdr';
      ...
      const status = last && (last.status||last.result||'');
      const pnl = last && ((last.profit??last.pnl)??0);
      const max = last && (last.maxStep??0);
      el.textContent = `| Cycles: ${n}${last?` | Last: ${String(status).toUpperCase()} P=${pnl} S=${max}`:''}`;
    }catch(e){}
  };
})();
...
try{ Object.defineProperty(window,'currentBetStep',{ configurable:true, get:()=>currentBetStep, set:v=>{ currentBetStep=v; } }); }catch(e){}
try{ Object.defineProperty(window,'maxStepInCycle',{ configurable:true, get:()=>maxStepInCycle, set:v=>{ maxStepInCycle=v; } }); }catch(e){}
...
SourceApp102SHA256=af2a2e41f7f5a2004b58a333c3ff63b852a15d108ffb12b3e6a6b92cd1fd0b7e
Header=OK
SSOT_VER=OK
InsertOnly=OK
Syntax=OK
