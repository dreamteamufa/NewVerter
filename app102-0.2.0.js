...
// @version 0.2.0
// @description Minute-Sync Candles + Smooth Chart + TF Switch + Panel Version update (PCS-8 v3.2)
...
// [BEGIN BLOCK:MTC_TF_V9]
(function MTC_TF_V9(){
 'use strict';
 if(!localStorage.__MTC_TF) localStorage.__MTC_TF='5s';
 const btn=document.createElement('button');
 btn.id='tfSwitch';
 const current=localStorage.__MTC_TF||'5s';
 btn.textContent='TF: '+current;
 ...
 btn.onclick=function(){
  ...
  let i=arr.indexOf(localStorage.__MTC_TF||'5s');
  i=(i+1)%arr.length;
  localStorage.__MTC_TF=arr[i];
  btn.textContent='TF: '+arr[i];
  MTC.drawCandles?.();
 };
 document.getElementById('InfoPanel')?.appendChild(btn);
})();
 // [END BLOCK:MTC_TF_V9];
...
(function PANEL_VERSION_V9(){
 ...
 if(panel){
  ...
  ver.textContent='Сборка: app102 v0.2.0';
  ver.style.fontSize='12px';
  ver.style.color='#ccc';
  panel.appendChild(ver);
  console.log('[INFO] Panel version updated → app102 v0.2.0');
 }
})();
...
(function PCS8_SELFAUDIT_V2(){
 ...
 if(!p||!g||g.parentElement!==p)
  throw new Error('[PCS-8][STOP] graphs not in InfoPanel');
