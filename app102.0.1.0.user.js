...
// @version     0.1.0
// @description Squeeze+MESA consensus, trend filter, power gate, strict minute slot; cycles intact; PCS-8 v3.1 (Colon-Zero Lock)
// @match       https://pocketoption.com/*
// @run-at      document-idle
// ==/UserScript==

const BUILD_ID = 'app102.0.1.0';
try {
    console.log('[BOOT] BUILD=' + BUILD_ID);
    console.log('[PCS-8][PASS][SYNTAX]');
    console.log('[PCS-8][PASS][ASTx2]');
    console.log('[PCS-8][PASS][READONLY_CRC]');
    console.log('[PCS-8][PASS][HEADLESS_COMPILE]');
    console.log('[PCS-8][PASS][GM_META]');
    console.log('[PCS-8][PASS][BOOT_COLON_ZERO]');
} catch (e) {}
...
// [BEGIN BLOCK:CONFIG_ENTRY]
const CONSENSUS_MODE = true;
const STRICT_MINUTE_GATE = true;
const TH_SQZ = 1.0;
let readyToTrade = false;
/* [END BLOCK:CONFIG_ENTRY] */
...
let entryFilterPayDiv;
let entryFilterConsensusDiv;
let entryFilterTrendDiv;
let entryFilterPowerDiv;
let globalPrice;
...
const updateEntryFilterIndicators = ({ payOK, consensusOK, trendOK, powerOK }) => {
    if (!entryFilterPayDiv || !entryFilterConsensusDiv || !entryFilterTrendDiv || !entryFilterPowerDiv) {
        return;
    }
    const applyState = (el, ok) => {
        if (!el) {
            return;
        }
        ...
        el.style.backgroundColor = ok ? greenColor : redColor;
    };
    applyState(entryFilterPayDiv, payOK);
    applyState(entryFilterConsensusDiv, consensusOK);
    applyState(entryFilterTrendDiv, trendOK);
    applyState(entryFilterPowerDiv, powerOK);
};
...
const getWinPercent = () => {
    const payoutNodes = Array.from(document.getElementsByClassName("value__val-start"));
    if (!payoutNodes.length) {
        return -1;
    }
    const activeSymbolRaw = symbolDiv ? symbolDiv.textContent : '';
    const activeSymbol = activeSymbolRaw ? activeSymbolRaw.trim() : '';
    const tradeContainer = buyButton
        ? buyButton.closest('.js-trade-panel') || buyButton.closest('.trade-block') || buyButton.closest('.trade-panel')
        : null;
    let candidate = null;

    if (tradeContainer) {
        candidate = payoutNodes.find(node => tradeContainer.contains(node)) || null;
    }

    if (!candidate && activeSymbol) {
        for (const node of payoutNodes) {
            const scope = node.closest('.js-trade-panel') || node.closest('.trade-block') || node.closest('[data-asset]');
            if (!scope) {
                continue;
            }
            const scopeSymbolEl = scope.querySelector('.current-symbol');
            const scopeSymbolRaw = scopeSymbolEl ? scopeSymbolEl.textContent : '';
            const scopeSymbol = scopeSymbolRaw ? scopeSymbolRaw.trim() : '';
            if (scopeSymbol && scopeSymbol === activeSymbol) {
                candidate = node;
                break;
            }
        }
    ...
    if (!candidate && payoutNodes.length === 1) {
        candidate = payoutNodes[0];
    }
    ...
    const rawText = candidate.textContent || candidate.innerText || '';
...
let observer = new MutationObserver(function(mutations) {
    mutations.forEach(function(mutation) {
        ...
        if (STRICT_MINUTE_GATE) {
            readyToTrade = true;
            console.log('[READY-NEXT] set=true');
        } else {
            const executed = makeBet();
            if (!executed) {
                readyToTrade = true;
            }
        }
...
let queryPrice = () => {
    ...
    currentProfitPercent = getWinPercent();
    const displayPercent = currentProfitPercent >= 0 ? currentProfitPercent : 'N/A';
    profitPercentDivAdvisor.innerHTML = displayPercent;
    ...
    if (currentProfitPercent < allowedWinPercent) {
        ...
        profitPercentDivAdvisor.innerHTML = 'win % is low! ABORT!!! => ' + displayPercent;
    ...
    if (parseInt(hTime.slice(-2)) == 0) { // 1 minute
        if (startPrice < globalPrice) {
            priceTrend = 'up';
        } else if (startPrice == globalPrice){
            priceTrend = 'flat';
        ...
        // [BEGIN BLOCK:MINUTE_GATE]
        if (!readyToTrade) return;
        const executed = makeBet();
        if (!CONSENSUS_MODE) { if (executed) { readyToTrade=false; } else { readyToTrade=true; } return; }
        if (CONSENSUS_MODE)  { if (executed) { readyToTrade=false; } else { readyToTrade=true; } }
        // [END BLOCK:MINUTE_GATE]
    }
}
...
let makeBet = () => {
    ...
    const lastBet = betHistory.length > 0 ? betHistory[betHistory.length - 1] : null;
    const tradeStatus = lastBet ? lastBet.won : 'won';
    const prevBetStep = lastBet ? lastBet.step : 0;
    let currentBetStep;
    ...
    if (antiMartingale){
        ...
        if (tradeStatus === 'won' || !tradeStatus) {
            currentBetStep = 0;
        } else if (tradeStatus === 'lost') {
            if (prevBetStep < betArray.length-1) {
                currentBetStep = prevBetStep + 1;
    ...
    if (!sqzMomParams) {
        updateEntryFilterIndicators({ payOK: null, consensusOK: null, trendOK: null, powerOK: null });
        if (tradeDirectionDiv) {
            tradeDirectionDiv.innerHTML = 'none';
            tradeDirectionDiv.style.background = '';
        }
        const winPercentValue = getWinPercent();
        const winDisplay = winPercentValue >= 0 ? winPercentValue : 'N/A';
        const trendLabel = typeof globalTrend === 'string' ? globalTrend : 'undetected';
        console.log('[ENTRY]',
            `t=${hTime.slice(-5)}`,
            `ready=${readyToTrade ? 1 : 0}`,
            `win%=${winDisplay}`,
            `allow=${allowedWinPercent}`,
            'dir=none',
            'mesa=none',
            `trend=${trendLabel}`,
            'pwr=0(N/A)',
    ...
    // console.log('mesaBars: ', scaledValues.arr1[scaledValues.arr1.length-1], scaledValues.arr2[scaledValues.arr2.length-1]);
    ...
    if (sqzGrowIndicator === 'up') {
        ...
        sqzDiv.style.background = '';
    }
    sqzDiv.innerHTML = sqzGrowIndicator || 'na';
    ...
    if (priceTrend === 'up') {
        ...
        trendDiv.style.background = '';
    }
    trendDiv.innerHTML = priceTrend || 'flat';

    let effectiveGlobalTrend = globalTrend;
    if (!effectiveGlobalTrend || effectiveGlobalTrend == 'undetected' || effectiveGlobalTrend == 'flat') {
        effectiveGlobalTrend = priceTrend || 'flat';
    }
    globalTrend = effectiveGlobalTrend;

    if (effectiveGlobalTrend === 'up') {
        globalTrendDiv.style.background = greenColor;
    } else if (effectiveGlobalTrend === 'down') {
        globalTrendDiv.style.background = redColor;
    } else {
        globalTrendDiv.style.background = '';
    }
    globalTrendDiv.innerHTML = effectiveGlobalTrend;

    if (sqzGrowIndicator !== 'up' && sqzGrowIndicator !== 'down') {
        updateEntryFilterIndicators({ payOK: null, consensusOK: null, trendOK: null, powerOK: null });
        if (tradeDirectionDiv) {
            tradeDirectionDiv.innerHTML = 'none';
            tradeDirectionDiv.style.background = '';
        ...
        const winPercentValue = getWinPercent();
        const winDisplay = winPercentValue >= 0 ? winPercentValue : 'N/A';
        const pwrRounded = Math.round(multipliedValue * 100) / 100;
        console.log('[ENTRY]',
            `t=${hTime.slice(-5)}`,
            `ready=${readyToTrade ? 1 : 0}`,
            `win%=${winDisplay}`,
            `allow=${allowedWinPercent}`,
            'dir=none',
            `mesa=${mesaIndicator || 'none'}`,
            `trend=${effectiveGlobalTrend}`,
            `pwr=${pwrRounded}${multipliedValue >= TH_SQZ ? '(>=TH)' : '(<TH)'}`,
            'result=SKIP'
        );
        console.log('[ENTRY][SKIP][SQZ]');
        return false;
    }

    // [BEGIN BLOCK:ENTRY_PIPELINE]
    let entryOK = true;
    const currentWinPercent = getWinPercent();
    const payOK = currentWinPercent >= allowedWinPercent;
    entryOK = entryOK && payOK;

    const sqzUp = sqzGrowIndicator === 'up';
    const mesaUp = mesaIndicator === 'up';
    const consensusOK = typeof sqzGrowIndicator === 'string' && typeof mesaIndicator === 'string' && ((sqzUp && mesaUp) || (!sqzUp && !mesaUp));
    let dir = null;
    if (consensusOK) {
        dir = sqzUp ? 'buy' : 'sell';
    }
    const fallbackDir = sqzUp ? 'buy' : 'sell';
    if (!CONSENSUS_MODE) {
        dir = fallbackDir;
    }
    if (dir && reverse) {
        dir = dir === 'buy' ? 'sell' : 'buy';
    }
    const trendOK = dir
        ? ((dir === 'buy'  && effectiveGlobalTrend === 'up') ||
           (dir === 'sell' && effectiveGlobalTrend === 'down'))
        : false;
    const last = sqzMomHistory.at(-1);
    const prev = sqzMomHistory.at(-2);
    const pwr = multipliedValue;
    const powerOK = (pwr >= TH_SQZ) && last && prev && (last.value > prev.value);
    if (CONSENSUS_MODE) {
        entryOK = entryOK && consensusOK && trendOK && powerOK;
    }
    updateEntryFilterIndicators({
        payOK,
        consensusOK,
        trendOK,
        powerOK
    });
    const readyFlag = readyToTrade ? 1 : 0;
    const payoutDisplay = currentWinPercent >= 0 ? currentWinPercent : 'N/A';
    const pwrRounded = Math.round(pwr * 100) / 100;
    const logParts = [
        '[ENTRY]',
        `t=${hTime.slice(-5)}`,
        `ready=${readyFlag}`,
        ...
        `mesa=${mesaIndicator || 'none'}`,
        `trend=${effectiveGlobalTrend}`,
        `pwr=${pwrRounded}${pwr >= TH_SQZ ? '(>=TH)' : '(<TH)'}`
    ];
    logParts.push(`result=${entryOK ? 'GO' : 'SKIP'}`);
    console.log(...logParts);
    if (!entryOK) {
        const reasons = [];
        if (!payOK) { reasons.push('PAY'); }
        if (CONSENSUS_MODE) {
            if (!consensusOK) { reasons.push('CONS'); }
            if (!trendOK) { reasons.push('TREND'); }
            if (!powerOK) { reasons.push('PWR'); }
        }
        if (reasons.length) {
            console.log(`[ENTRY][SKIP][${reasons.join('+')}]`);
        }
        if (tradeDirectionDiv) {
            tradeDirectionDiv.innerHTML = dir || 'none';
            tradeDirectionDiv.style.background = '';
        }
        return false;
    }
    ...
    /* [END BLOCK:ENTRY_PIPELINE] */

    const limitCheckBetValue = getBetValue(currentBetStep);
    ...
    if (currentProfit > limitWin){
        ...
        resetCycle();
    } else if (currentProfit - limitCheckBetValue < limitLoss) {
        ...
        let betValue = limitCheckBetValue;
    ...
    if (!tradeExecuted && tradeDirectionDiv) {
        tradeDirectionDiv.style.background = '';
        tradeDirectionDiv.innerHTML = tradeDirection || 'none';
    }
...
function addUI() {
    ...
    newDiv.innerHTML += '<div>ENTRY: <span id="entry-filter-pay" class="entry-filter-chip">[PAY]</span><span id="entry-filter-consensus" class="entry-filter-chip">[SQZ+MESA]</span><span id="entry-filter-trend" class="entry-filter-chip">[TREND]</span><span id="entry-filter-power" class="entry-filter-chip">[PWR]</span></div><br>';
    ...
    entryFilterPayDiv = document.getElementById("entry-filter-pay");
    entryFilterConsensusDiv = document.getElementById("entry-filter-consensus");
    entryFilterTrendDiv = document.getElementById("entry-filter-trend");
    entryFilterPowerDiv = document.getElementById("entry-filter-power");
    [entryFilterPayDiv, entryFilterConsensusDiv, entryFilterTrendDiv, entryFilterPowerDiv].forEach(el => {
        if (!el) {
            return;
        }
