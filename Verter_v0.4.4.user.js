// ==UserScript==
// @name         Verter_v0.4.4
// @description  Auto-trade + Signals (EMA3/9, EMA9/15, SQZ) + CLOSE via legacy Trades observer (guard+dedupe) + BalanceStable(2s) + DockBox (+50%) + Stats (Wager on DOM close) + One-At-A-Time Gate + PCS-8 v3.1 (Colon-Zero Lock) + Cycles history viewer in DockBox (colored)
// @version      0.4.4
...
let vx_curPnl = 0;                 // PnL текущего цикла (viewer)
let vx_maxStep = 0;                // макс. шаг в текущем цикле
let vx_stats = [];                 // массив итогов завершённых циклов (viewer)
let vx_total = 0;                  // сумма по vx_stats
let vx_prevBal = null;             // баланс для расчёта Δ, если pnl=null
let vx_sum = null, vx_hist = null; // узлы DockBox
let vx_cycleLatched = false;       // защита от дубля записи цикла

function vx_r2(x){ return Math.round((+x)*100)/100; }
function vx_num(v){ return (v==null || isNaN(+v)) ? 0 : +v; }
function vx_delta(finalBal){ const d = (vx_prevBal==null)?0:(vx_num(finalBal)-vx_prevBal); vx_prevBal=vx_num(finalBal); return vx_r2(d); }

function vx_activeLimits(){
  // приоритетные «сквозные» переменные, если они уже собраны в одном числе
  if (typeof window.limitWin==='number' && typeof window.limitLoss==='number') return {win:+window.limitWin, loss:+window.limitLoss};
  // иначе выбрать из существующих наборов 1/2/3 по текущему профилю/сетке ставок
  const id = (typeof window.betArrayId==='number' ? window.betArrayId : 1);
  const L1 = {win: (typeof limitWin1  !=='undefined'? +limitWin1  : 0), loss: (typeof limitLoss1  !=='undefined'? +limitLoss1  : 0)};
  const L2 = {win: (typeof limitWin2  !=='undefined'? +limitWin2  : 0), loss: (typeof limitLoss2  !=='undefined'? +limitLoss2  : 0)};
  const L3 = {win: (typeof limitWin3  !=='undefined'? +limitWin3  : 0), loss: (typeof limitLoss3  !=='undefined'? +limitLoss3  : 0)};
  return (id===2?L2:(id===3?L3:L1));
}

function vx_resetCycleView(){
  vx_stats.push(vx_r2(vx_curPnl));
  vx_total = vx_r2(vx_stats.reduce((a,b)=>a+b,0));
  vx_curPnl = 0; vx_maxStep = 0; vx_cycleLatched = false;
}

function vx_span(status,p,maxS){
  const s=document.createElement('span');
  s.className='vx-cycle '+(status==='win'?'win':'lose');
  s.textContent=`${status.toUpperCase()} P=${vx_r2(p)} S=${maxS}`;
  return s;
}

function vx_renderSummary(){
  if (vx_sum) vx_sum.textContent=`| MaxStep: ${vx_maxStep} | Cycles: ${vx_stats.length} | TotalProfit: ${vx_total}`;
}
// /* [END BLOCK:CYCLES:STATE] */

// [BEGIN BLOCK:CYCLES:STYLE]
(function(){
  const css=document.createElement('style');
  css.textContent=`
    #vx-cycles-sum,#vx-cycles-hist{margin-left:8px}
    .vx-cycle{margin-left:8px}
    .vx-cycle.win{color:#2ecc71}   /* зелёный текст */
    .vx-cycle.lose{color:#e74c3c}  /* красный текст */
  `;
  document.head.appendChild(css);
})();
// /* [END BLOCK:CYCLES:STYLE] */
...
(function(){
  // ленивое оборачивание onClose, если он появится позже
  function tryWrap(){
    try{
      if(!window.SessionStats || typeof SessionStats.onClose!=='function' || SessionStats.onClose.__vx_wrapped) return false;
      const orig = SessionStats.onClose.bind(SessionStats); // сигнатура: (type, pnl, finalBal, amount)
      function wrapped(type, pnl, finalBal, amount){
        const ret = orig(type, pnl, finalBal, amount);
        try{
          // шаг серии
          const step = (typeof window.currentBetStep==='number') ? (window.currentBetStep|0) : 0;
          if(step>vx_maxStep) vx_maxStep=step;

          // PnL сделки: используем pnl, если он числовой, иначе Δбаланса
          const dp = vx_num(pnl) || vx_delta(finalBal);
          vx_curPnl = vx_r2(vx_curPnl + (dp||0));

          // активные лимиты берём из существующих переменных (без дублей)
          const L = vx_activeLimits();
          const hitWin  = (typeof L.win  ==='number') && (vx_curPnl >= +L.win);
          const hitLoss = (typeof L.loss ==='number') && (vx_curPnl <= +L.loss); // если loss отрицательный — условие корректно

          // фиксируем завершение цикла один раз
          if(!vx_cycleLatched && (hitWin || hitLoss)){
            const status = hitWin ? 'win' : 'lose';
            if(vx_hist) vx_hist.appendChild(vx_span(status, vx_curPnl, vx_maxStep));
            vx_cycleLatched = true;
          }

          // детект начала нового цикла по возврату шага (или первой новой сделке после лока)
          if(vx_cycleLatched && step<=1){ vx_resetCycleView(); }

          vx_renderSummary();
          if(window.DockBox && DockBox.renderHeader) DockBox.renderHeader();
        }catch(e){}
        return ret;
      }
      wrapped.__vx_wrapped = true;
      SessionStats.onClose = wrapped;
      return true;
    }catch(e){ return false; }
  }
  if(!tryWrap()){
    const t0=Date.now(); const iv=setInterval(()=>{ if(tryWrap() || Date.now()-t0>60000) clearInterval(iv); },500);
  }
})();
...
// [BEGIN BLOCK:CYCLES:DOCKBOX_BOOT]
(function(){
  function attach(){
    const host = document.querySelector('.verter-dock-header,[data-verter-header]');
    if(!host) return false;
    if(!vx_sum){ vx_sum=document.createElement('span'); vx_sum.id='vx-cycles-sum'; host.appendChild(vx_sum); }
    if(!vx_hist){ vx_hist=document.createElement('span'); vx_hist.id='vx-cycles-hist'; host.appendChild(vx_hist); }
    vx_renderSummary();
    return true;
  }
  if(attach()) return;
  const mo=new MutationObserver(()=>{ if(attach()) try{mo.disconnect();}catch(e){} });
  mo.observe(document.documentElement,{childList:true,subtree:true});
  let tries=0; const iv=setInterval(()=>{ try{
    if(window.DockBox && DockBox.renderHeader) DockBox.renderHeader();
    if(attach() || ++tries>40) clearInterval(iv); // ~20s
  }catch(e){} },500);
})();
// /* [END BLOCK:CYCLES:DOCKBOX_BOOT] */
