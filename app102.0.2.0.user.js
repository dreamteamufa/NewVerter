...
// @version     0.2.0
// @description Candles preview (DOM OHLC) for validation; NO trading logic changes; PCS-8 v3.1
...
const CANDLE_PREVIEW_W = 320;               // ширина canvas в px
const CANDLE_PREVIEW_H = 96;                // высота canvas в px
  ...
  try{
    ...
    function ensurePanel(){
      ...
      if (!box){
        ...
        box.style.cssText = 'font:12px/1.2 system-ui,sans-serif;margin:6px 0 0 0';
        ...
        canvas.style.cssText = 'width:100%;max-width:100%;display:block;background:#0b0e13;border:1px solid #1f2630;border-radius:8px';
    ...
    function draw(){
      ...
      const pad = (hi-lo)*0.05 || 0.0001; lo-=pad; hi+=pad;
    ...
    function parseTooltip(){
      const el = document.querySelector('.tooltip-text, .chart-tooltip, [class*="tooltip"]');
      const txt = el && el.textContent ? el.textContent : '';
      const m = txt.match(/O=([\d.,]+).*H=([\d.,]+).*L=([\d.,]+).*C=([\d.,]+)/i);
      if (!m) return null;
    ...
    const obs = new MutationObserver(() => {
      ...
      if (key !== lastKey){
        ...
        draw();
        try{ console.log('[M1][BOT-READ]', `O=${c.O} H=${c.H} L=${c.L} C=${c.C}`); }catch(_){}
      }else{
