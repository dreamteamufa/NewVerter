...
// @version     0.1.1
// @description Candles preview from DOM OHLC in InfoPanel; full OHLC console log; NO trading logic changes; PCS-8 v3.1
...
// [BEGIN BLOCK:CANDLE_PREVIEW_CONFIG]
const CANDLE_PREVIEW_ENABLED = true;   // включить мини-свечи
const CANDLE_PREVIEW_COUNT   = 60;     // сколько последних свеч рисовать
const CANDLE_PREVIEW_W       = 320;    // ширина canvas
const CANDLE_PREVIEW_H       = 96;     // высота canvas
// [END BLOCK:CANDLE_PREVIEW_CONFIG]
...
// [BEGIN BLOCK:CANDLE_PREVIEW_PANEL]
(() => {
  try{
    if (!CANDLE_PREVIEW_ENABLED) return;

    // ——— приоритет привязки: наш правый оверлей (#graphs), затем DockBox/InfoPanel
    function ensurePanel(){
      const host =
        document.querySelector('#graphs') ||        // <- фикс видимости в твоём UI
        document.querySelector('#DockBox') ||
        document.querySelector('#InfoPanel') ||
        document.body;
      if (!host) return null;

      let box = document.getElementById('ip-candle-preview');
      if (!box){
        box = document.createElement('div');
        box.id = 'ip-candle-preview';
        box.style.cssText = 'font:12px/1.2 system-ui,sans-serif;margin:6px 0;';
        const title = document.createElement('div');
        title.textContent = 'Candles preview (bot read)';
        title.style.cssText = 'opacity:.75;margin:0 0 4px 0';
        const canvas = document.createElement('canvas');
        canvas.id = 'ip-candle-canvas';
        canvas.width  = CANDLE_PREVIEW_W;
        canvas.height = CANDLE_PREVIEW_H;
        canvas.style.cssText = 'width:100%;display:block;background:#0b0e13;border:1px solid #1f2630;border-radius:8px';
        box.appendChild(title);
        box.appendChild(canvas);
        host.appendChild(box);
      }
      return document.getElementById('ip-candle-canvas');
    }

    // ——— локальное хранилище OHLC
    const candles = []; // [{t, O,H,L,C}]
    // создать панель сразу, даже без данных
    ensurePanel();

    const byMinuteKey = (t) => { const d=new Date(t); d.setSeconds(0,0); return d.toISOString(); };

    // ——— отрисовка мини-свечей
    function draw(){
      const cv = ensurePanel(); if (!cv) return;
      const ctx = cv.getContext('2d'), W=cv.width, H=cv.height;
      ctx.clearRect(0,0,W,H);

      const data = candles.slice(-CANDLE_PREVIEW_COUNT);
      if (!data.length){
        ctx.font = '12px system-ui';
        ctx.fillStyle = '#9aa4b2';
        ctx.fillText('нет данных OHLC', 8, 16);
        return;
      }
      ...
      const left=4, right=2, top=2, bottom=2;
      const innerW=W-left-right, innerH=H-top-bottom;
      const xStep = innerW/Math.max(1,data.length);
      const y = v => top + (hi - v) * (innerH / (hi - lo));

      for (let i=0;i<data.length;i++){
        const c=data[i], x=left+i*xStep+xStep*0.5;

        // тени
        const p = new Path2D();
        p.moveTo(x, y(c.H)); p.lineTo(x, y(c.L));
        ctx.lineWidth=1; ctx.strokeStyle='#9aa4b2'; ctx.stroke(p);

        // тело
        const up = c.C >= c.O;
        const bodyW = Math.max(1, Math.floor(xStep*0.55));
    ...
    // ——— парсинг tooltip из DOM (усиленные селекторы + лог)
    function parseTooltip(){
      const el =
        document.querySelector('.chart-tooltip, .highcharts-tooltip, .tv-lightweight-charts__crosshair') ||
        document.querySelector('.tooltip-text,[class*="tooltip"]');
      const txt = el && el.textContent ? el.textContent : '';
      const m = txt.match(/O\s*=?\s*([\d.,]+).*H\s*=?\s*([\d.,]+).*L\s*=?\s*([\d.,]+).*C\s*=?\s*([\d.,]+)/i);
      if (!m){ console.debug('[CANDLE-PREVIEW] OHLC tooltip not found'); return null; }
      const toNum = s => parseFloat(String(s).replace(',', '.'));
      return { O:toNum(m[1]), H:toNum(m[2]), L:toNum(m[3]), C:toNum(m[4]) };
    }

    // ——— обновление при изменениях DOM + полный лог свечей
    let lastKey = null;
    const obs = new MutationObserver(() => {
      const c = parseTooltip(); if (!c) return;

      const key = byMinuteKey(Date.now());
      if (key !== lastKey){
        // новая закрытая минута
        candles.push({ t:key, ...c });
        lastKey = key;
        if (candles.length > 1000) candles.splice(0, candles.length-1000);
        draw();
        // лог OHLC закрытой свечи
        try{ console.log('[M1][BOT-READ][CLOSE]', `t=${key} O=${c.O} H=${c.H} L=${c.L} C=${c.C}`); }catch(_){ }
      }else{
        // текущая формирующаяся свеча: H/L/C трекаем и логируем
        ...
        if (cur){
          ...
          cur.C=c.C;
          draw();
          try{ console.log('[M1][BOT-READ][LIVE]',  `t=${key} O=${cur.O} H=${cur.H} L=${cur.L} C=${cur.C}`); }catch(_){ }
        }else{
          // первая инициализация текущей минуты
          candles.push({ t:key, ...c });
          draw();
          try{ console.log('[M1][BOT-READ][INIT]',  `t=${key} O=${c.O} H=${c.H} L=${c.L} C=${c.C}`); }catch(_){ }
        }
      }
    });
...
// [END BLOCK:CANDLE_PREVIEW_PANEL]
