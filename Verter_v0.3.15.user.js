...
// @name         Verter_v0.3.15
// @version      0.3.15
// @description  FAST5S mode on top of v0.3.14 + keep existing UI payout & panel height
...
function onTickS5(price, ts){
  ...
  if (!c || (ts - c.ts) >= 5000){
    ...
    if (CandleS5.candles.length > 600) CandleS5.candles.shift(); // хранить ~50 мин S5
  } else {
...
let __fastMode = 'FAST5S'; // по умолчанию быстрый режим
const __fastModeListeners = [];
function getActiveMode(){ return __fastMode; }
function setActiveMode(name){
  const next = (name === 'M1' ? 'M1' : 'FAST5S');
  if(__fastMode === next) return;
  __fastMode = next;
  for(let i=0;i<__fastModeListeners.length;i++){
    try{ __fastModeListeners[i](__fastMode); }catch(e){}
  }
}
function onModeChange(cb){ if(typeof cb==='function') __fastModeListeners.push(cb); }
...
(function () {
  ...
  function getDecisionCandles(){
    if (getActiveMode() === 'FAST5S'){
      // использовать краткий контекст S5
      return CandleS5.candles;        // последний элемент — текущая S5
    } else {
      // как было в 0.3.14: использовать M1/M5 из существующих модулей
      const M1 = Candles.getState();
      const candlesM1 = M1 && Array.isArray(M1.candlesM1) ? M1.candlesM1 : [];
      return M1.candles || candlesM1; // оставить ровно так, как в базе
    }
  }
...
`);
  ...
  var DockBox=(function(){
    ...
    modeRow.style.cssText = 'display:flex;align-items:center;gap:6px;margin-top:6px;';
    ...
    const modeBtnCSS = 'height:26px;padding:2px 10px;background:#2a2a2a;color:#eee;border:1px solid #666;border-radius:6px;cursor:pointer;font:12px monospace;';
    [btnFast, btnM1].forEach(btn=>{
      btn.style.cssText = modeBtnCSS;
      btn.addEventListener('mouseenter', e=>{ e.currentTarget.style.background = '#3a3a3a'; });
      btn.addEventListener('mouseleave', e=>{ if(!e.currentTarget.disabled) e.currentTarget.style.background = '#2a2a2a'; });
      ['mousedown','click'].forEach(evt=>{
        btn.addEventListener(evt, ev=>ev.stopPropagation());
      });
    });
    function syncModeButtons(){
      const mode = getActiveMode();
      [btnFast, btnM1].forEach(btn=>{ btn.disabled = false; btn.style.opacity = '1'; });
      if(mode === 'FAST5S'){
        btnFast.disabled = true;
        btnFast.style.background = '#3a3a3a';
        btnFast.style.opacity = '0.8';
        btnM1.style.background = '#2a2a2a';
      } else {
        btnM1.disabled = true;
        btnM1.style.background = '#3a3a3a';
        btnM1.style.opacity = '0.8';
        btnFast.style.background = '#2a2a2a';
      }
    }
    btnFast.addEventListener('click', ()=>{ setActiveMode('FAST5S'); syncModeButtons(); });
    btnM1.addEventListener('click', ()=>{ setActiveMode('M1'); syncModeButtons(); });
    onModeChange(syncModeButtons);
    syncModeButtons();
    modeRow.append(modeLabel, btnFast, btnM1);
    ...
    try {
      tickSource.subscribe(function onTickForS5(p){
        const now = Date.now();
        onTickS5(p, now);
      }, 100); // период ≥100мс; не занижать ниже базы
    } catch(e){ /* без побочных эффектов */ }
  ...
  var SignalEngine=(function(){
    ...
    function updateIndicators(){
      ...
      var candles=[];
      if(mode==='FAST5S'){
        var s5Source=getDecisionCandles()||[];
        candles=s5Source.slice(-260).map(function(c){
          return { tOpen:c.ts||0, O:c.open, H:c.high, L:c.low, C:c.close };
        });
      }else{
        candles=st.candlesM1.slice(-260);
        if(st.cur){
          candles=candles.concat([{tOpen:st.cur.tOpen||0,O:st.cur.O,H:st.cur.H,L:st.cur.L,C:st.cur.lastP}]);
        }
      }
      candles=candles.filter(function(c){ return c && isFinite(c.O) && isFinite(c.H) && isFinite(c.L) && isFinite(c.C); });
      if(!candles.length){
        state.candles=[];
        state.closes=[];
        state.price=null;
  ...
  // [BEGIN BLOCK:Fast5S:Guards]
  let lastFast5SCandleTs = null;
  function canOpenFast5STrade(){
    if(getActiveMode() !== 'FAST5S') return true;
    const cur = CandleS5.cur;
    if(!cur) return false;
    return lastFast5SCandleTs !== cur.ts;
  }
  function markFast5STrade(){
    if(getActiveMode() !== 'FAST5S') return;
    const cur = CandleS5.cur;
    if(cur) lastFast5SCandleTs = cur.ts;
  }
  onModeChange(function(){ lastFast5SCandleTs = null; });
  /* [END BLOCK:Fast5S:Guards] */
  ...
  (function(){
    ...
    setInterval(function(){
      ...
      try{ console.log('[TOP HIT] name=%s side=%s score=%s mode=%s', sig, side, best.score.toFixed(2), getActiveMode()); }catch(e){}
      ...
      if(typeof MARTIN_ENABLED==='undefined' || !MARTIN_ENABLED){
        ...
        try{ console.log('[ORDER try] sig=%s side=%s step=%d mode=%s', sig, side, pendingStepForLog, getActiveMode()); }catch(e){}
...
FAST5S enabled (local S5 aggregator + mode switch)
Unchanged: PauseCtl, DataFeed/BalanceStable(2s), DockBox payout UI, SessionStats, CandleChart, TradeRowObserver
