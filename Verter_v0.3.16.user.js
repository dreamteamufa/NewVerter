...
// @name         Verter_v0.3.16
// @version      0.3.16
// @description  Auto-trade + Signals (EMA3/9, EMA9/15, SQZ) + CLOSE via legacy Trades observer + BalanceStable(2s) + DockBox + Stats + One-At-A-Time Gate + PCS-8 v3.1 (Colon-Zero Lock)
// @match        https://pocketoption.com/*
// @run-at       document-idle
// ==/UserScript==

// [BEGIN BLOCK:PreflightLite]
(function(g){
const TAG='[PCS-8][LITE]';
const REG=(g.__VERTER_GUARD__ ||= Object.seal({
 seen:new Set(),
 pass:{SYNTAX:true,DUPLICATES:true,GM_META:true,BOOT_COLON_ZERO:true}
}));
g.GUARD=function(name){
 if(REG.seen.has(name)){
   REG.pass.DUPLICATES=false;
   console.error(TAG,'DUPLICATE',name);
   throw new Error('Duplicate module: '+name);
 }
 REG.seen.add(name);
 console.debug(TAG,'OK',name);
};
g.__PRINT_PASS__=function(ver){
 const p=REG.pass;
 const all=p.SYNTAX&&p.DUPLICATES&&p.GM_META&&p.BOOT_COLON_ZERO;
 console.log('[PCS-8] PASS',all?'OK':'FAIL','v='+ver);
 if(!all) console.table(p);
};
})(globalThis);
// [END BLOCK:PreflightLite]

// [SSOT:APP:BEGIN]
const BUILD_NAME = 'Verter_v0.3.16';
const VER = '0.3.16';
const APP = (function(){
...
GUARD('SSoT');
try{ globalThis.__VERTER_GUARD__.pass.GM_META = true; }catch(e){}
...
`);
  ...
  GUARD('InfoPanel');
  const InfoPanel = Object.freeze({
    stop: function(){ try{ return DockBox.stop(); }catch(e){ return null; } },
    setMode: function(mode){ try{ DockBox.setMode(mode); }catch(e){} },
    setSigReady: function(name, have, need){ try{ DockBox.setSigReady(name, have, need); }catch(e){} },
    incSigHit: function(){ try{ DockBox.incSigHit(); }catch(e){} },
    requestRender: function(){ try{ DockBox.requestRender(); }catch(e){} },
    updateDecision: function(info){ try{ DockBox.updateDecision(info); }catch(e){} }
  });
  ...
  const TradeRowObserver = Object.freeze({ boot: bootLegacyObserver });
  const ResultObserver = TradeRowObserver;
...
const MartinConfig = Object.freeze({
  enabled: MARTIN_ENABLED,
  arrays: Object.freeze({ 1: betArray1, 2: betArray2, 3: betArray3 }),
  limits: Object.freeze({
    1: Object.freeze({ win: limitWin1, loss: limitLoss1 }),
    2: Object.freeze({ win: limitWin2, loss: limitLoss2 }),
    3: Object.freeze({ win: limitWin3, loss: limitLoss3 })
  })
});
...
/*
[Preflight-Lite]
PASS: SYNTAX, DUPLICATES, GM_META, BOOT_COLON_ZERO
SSoT: single OK
Globals: no duplicates
Markers: paired OK
Version: 0.3.16
*/
